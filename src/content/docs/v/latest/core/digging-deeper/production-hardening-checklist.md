...
- [ ] **Decide who you trust for `X-Forwarded-For`.** **Since 4.13.0 `http.trustProxy` defaults to `false`**, so `request.detectIp()`, `middleware.ipFilter()` and IP-scoped rate limiting use the socket address. **Prefer a trusted-proxy CIDR list over bare `true`** — `true` trusts the whole `X-Forwarded-For` chain, including whatever the client itself prepended if your edge only appends (the common nginx/ALB/CDN case); a proxy list (`"10.0.0.0/8"`) walks only past your own known proxies. Setting `true` without a proxy that overwrites the header lets any client choose its own IP and makes per-IP rate limits and IP allowlists bypassable. `X-Real-IP` is honoured only under `trustProxy: true` — a bounded `trustProxy` reads `X-Forwarded-For` instead. **Since 5.2 a hop count (`trustProxy: 2`) — or any other number — is refused with a `TypeError` at boot** rather than silently evaluating as "trust nothing"; migrate it to a proxy list or an `(address, hop) => boolean` predicate. See [Security → `trustProxy` shapes](./security.md#trustproxy-shapes) and [Known Limitations → transport & proxy trust](./known-limitations.md).
