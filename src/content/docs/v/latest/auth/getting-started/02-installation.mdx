...
`@warlock.js/auth` ships the token, family-association, and provider Cascade migrations and two CLI commands. Wire them in `warlock.config.ts`:
...
`authMigrations` is an array of package migrations, including additive 5.19 token-family migrations. If you already have a `migrations` array, spread them in: `migrations: [...mine, ...authMigrations]`.
