...
:::note
The guards live on the server-only subpath `@warlock.js/web/session`, so they never reach the client bundle. Client code reads the user with `useUser()` from `@warlock.js/web`.
:::
