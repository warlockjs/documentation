...
## `metadata()` may only read resolved keys

`metadata()` runs before the shell flushes; a deferred key resolves only
...
There is no way to make `metadata()` wait for a deferred value. If the
