...
- `flush()` without a `globalPrefix` runs `FLUSHDB` and clears the whole connected database; set a `globalPrefix` to flush only your keys
- `removeNamespace("users")` matches `users` and `users.*` only (never `users2.*`) and deletes in batched `UNLINK`
- `increment` (`INCRBY`), `pull` (`GETDEL`) and `update` (Lua compare-and-set) are atomic across servers, and keep the key's TTL
...
`removeNamespace(ns)` walks matching keys with a non-blocking `SCAN` cursor (`scanIterator`) rather than the blocking `KEYS` command, so clearing a namespace on a large keyspace doesn't stall the single-threaded Redis event loop for other tenants. The namespace string is also glob-escaped (`*`, `?`, `[`, `\`) before it's used to build the match pattern, so untrusted input in the namespace can't widen the match beyond its own prefix.
...
- **Credentials in logs?** Connection/command failures are logged with the raw error message redacted (`scheme://user:pass@…` → `scheme://[REDACTED]@…`) — a failed connect never prints your Redis URL's password.
