...
- **Atomic ops**: `increment` is one upsert that keeps `expires_at`; `pull` is `DELETE … RETURNING`; `update` is a compare-and-set with bounded retries (the callback may run more than once). All are safe across servers.
- **Tags**: the tag index row is updated with a one-statement JSON-array merge.
- **Expiry clock**: `expires_at` is computed on the database clock, so app-server clock skew doesn't shift TTLs.
- **Pruning**: expired rows are removed by `prune(limit = 1000)`, which returns the number deleted. `set()` also runs it in the background on about 1 in 200 writes; call it yourself from a scheduled job on unique-key-heavy workloads.
