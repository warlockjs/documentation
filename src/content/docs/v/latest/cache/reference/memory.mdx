...
- Not shared between processes or servers — with more than one server use [Redis](/v/latest/cache/redis/) or [Postgres](/v/latest/cache/pg/)
- Keys are stored flat: `get("users")` after `set("users.1", …)` is a miss, and `maxSize` counts entries (evicting in LRU order)
- `increment`, `pull` and `update` are atomic within the process only
