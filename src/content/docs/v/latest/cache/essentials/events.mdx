...
> **For aggregate observability — hit rate, latency percentiles, per-driver breakdowns — use the built-in collector instead of wiring everything by hand: see [Built-in Metrics](/v/latest/cache/metrics/).** Use the raw event listeners below when you need per-event reactions (alerts, audit logs) that the collector doesn't cover.
...
See [Best Practices](/v/latest/cache/best-practices/) for more monitoring patterns.
