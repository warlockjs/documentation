...
slug: v/latest/cache/comparison
---
...
**@warlock.js/cache:** ✅ Cross-server atomic `increment`/`pull`/`update` on Redis and Postgres (in-process on the others), cross-driver conditional writes
```ts
...
await cache.update<User>('user.1', (u) => ({ ...u, seen: Date.now() })); // compare-and-set on Redis/Postgres, chain-locked elsewhere
```
