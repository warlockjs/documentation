...
- File driver: a key/namespace resolves outside the configured cache root even after percent-encoding — `"Cache key resolves outside the cache directory: ..."`. Last line of defense against path traversal; shouldn't happen from normal keys. See [File Cache Driver — Key Safety](/v/latest/cache/file/#key-safety).
