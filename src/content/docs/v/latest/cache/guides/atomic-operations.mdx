...
On Redis, `increment` uses the native `INCRBY` command, and on Postgres a single upsert statement — atomic across servers even with thousands of concurrent requests. Both keep the key's remaining TTL, so a fixed-window counter can't turn permanent.
...
For structured values (not just counters), `update()` does read-transform-write atomically: an optimistic compare-and-set on Redis and Postgres (across servers), and a per-key chain lock elsewhere (one process):
...
## Which drivers are atomic across servers

| Driver | `increment` / `decrement` | `pull` | `update` / `merge` |
| --- | --- | --- | --- |
| `redis` | Cross-server (`INCRBY`, keeps TTL) | Cross-server (`GETDEL`) | Cross-server (Lua compare-and-set, bounded retries) |
| `pg` | Cross-server (one upsert, keeps expiry) | Cross-server (`DELETE … RETURNING`) | Cross-server (compare-and-set, bounded retries) |
| `memory` / `memoryExtended` / `lru` / `mock` | One process | One process | One process (serialized per key) |
| `file` | One process (serialized per key) | One process (serialized per key) | Not supported (throws) |

Under contention, `update()` callbacks on `redis` and `pg` can run more than once, so keep them free of side effects. `pull()` hands a one-time value to exactly one caller on `redis` and `pg`. With several servers, use one of those two; `memory` is per-process.
...
1. **Use Redis or Postgres for distributed systems.** They are the drivers with cross-server atomicity.
...
- **Rate limiter lets too many requests through?** Check you're using the Redis or Postgres driver; memory drivers are per-process.
