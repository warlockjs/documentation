...
|------------|-----|
| No args | `prefix` |
| Any args | `${prefix}.${hash}` — the first 16 hex chars of the sha1 of a stable JSON encoding of the args |
| `Map`, `Set`, functions, symbols, class instances, circular refs | `CacheConfigurationError` |

The hash keeps distinct args on distinct keys: `profile("1.private", "")` and `profile("1", "private")`, or `{ q: "x" }` and `("q", "x")`, never share an entry. Object keys are sorted, `Date` becomes an ISO string, `undefined` and `null` are distinct, and `bigint` is encoded as a decimal string.

**Examples:**

```ts
getUser()                             // "user"
getUser(42)                           // "user.<16 hex chars>"
searchBy({ q: "hello" })              // "search.<16 hex chars>"
```

The exact hash is not part of the contract. Don't build keys by hand; use `.invalidate(...args)`.

:::caution Upgrading to 5.20
The auto-key format changed (it was dot-joined args). Entries written by earlier versions are never read again, so every `cached()` call misses once and repopulates. Old entries expire by TTL, or `flush()` clears them.
:::

## Caveats

- **Order matters.** `getOrder(42, "abc")` and `getOrder("abc", 42)` produce different keys.
- **`Date` → ISO string.** Two `Date` objects at the same millisecond share a key — correct.
- **`Map`, `Set` and class instances throw.** They can't be encoded stably. Use the options form with a custom `key` fn.
- `.invalidate()` uses the configured `driver`, the same one the wrapper reads and writes.
...
- **`CacheConfigurationError: could not derive an auto-key from args`** — your args include something that can't be encoded stably (circular ref, `Map`, `Set`, function, class instance). Switch to the options form with a custom `key` fn that picks JSON-safe fields.
