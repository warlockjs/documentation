...
- **[`orchestrator`](/v/latest/ai/orchestration/run-orchestrator/)** — when the session matters across runs (durable session state, drift detection, resume).
...
> **⚠ Changed in 4.3.0** — `snapshotStore` is now typed `SnapshotStore` and the fallback is `ai.config({ defaultSnapshotStore })`, not `defaultStore` / a `CacheDriver`. See the migration note in [Persist AI data](/v/latest/ai/reliability/persist-ai-data/).

Signature drift detection throws `SupervisorDriftError` on shape mismatch. `force: true` bypasses for safe edits. See [Persist AI data](/v/latest/ai/reliability/persist-ai-data/).
...
`observe?: boolean | Observer` routes a supervisor's completed report through the same generic `Observer` seam every other flow uses. `true` sends it to the globally registered observers even when observe-all is off; `false` opts the run out; a flow-local `Observer` collects only this run's report. See [the Observer seam](/v/latest/ai/observability/observe-seam/) for the value semantics.

## Related

- [Run agent](/v/latest/ai/agents/run-agent/) — dispatchable units.
- [Run workflow](/v/latest/ai/orchestration/run-workflow/) — when steps are known up front.
- [Run orchestrator](/v/latest/ai/orchestration/run-orchestrator/) — when the session matters across runs.
- [Persist AI data](/v/latest/ai/reliability/persist-ai-data/) — snapshot store and resume.
- [Define tools](/v/latest/ai/tools/define-tools/) — tool artifacts side-channel.
- [Attach middleware](/v/latest/ai/reliability/attach-middleware/) — `semanticCache` fits under each agent's middleware.
