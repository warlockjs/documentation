...
See [Handle errors](/v/latest/ai/reliability/handle-errors/).
...
See [Persist AI data](/v/latest/ai/reliability/persist-ai-data/) for snapshot stores, drift detection, and recovery patterns.
...
- **Unknown shape at author time** — reach for [`ai.planner()`](/v/latest/ai/architecture-concepts/planner/) (LLM generates the plan) or model as a supervisor.
- **Quality loop until goal met** — `ai.supervisor()` with `evaluate`.
- **Multi-turn conversation with persistent session** — [`ai.orchestrator()`](/v/latest/ai/orchestration/run-orchestrator/).
...
- [Run agent](/v/latest/ai/agents/run-agent/) — agents inside steps.
- [Run supervisor](/v/latest/ai/orchestration/run-supervisor/) — the next rung up.
- [Run orchestrator](/v/latest/ai/orchestration/run-orchestrator/) — durable sessions across runs.
- [Persist AI data](/v/latest/ai/reliability/persist-ai-data/) — snapshot resume.
- [Handle errors](/v/latest/ai/reliability/handle-errors/) — `WorkflowError` subclasses.
