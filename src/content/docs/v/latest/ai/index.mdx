...
Beyond the ladder, 4.3.0 adds `ai.planner()` (an LLM generates an ordered plan over your capabilities, then runs it), `ai.memory()` (working + semantic recall across turns), and a separate observability package, `@warlock.js/ai-panoptic`, that exports run traces to OpenTelemetry, Langfuse, the console, a file, or a queryable store.
