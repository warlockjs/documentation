...
- [Use AI tools](/v/latest/ai/tools/use-ai-tools/) — the built-in web / HTTP / calculator / date-time belt you can consume or expose.
- [Use a workspace](/v/latest/ai/tools/use-workspace/) — a workspace whose `tools.all()` is a natural `serve` source.
- [Run agent](/v/latest/ai/agents/run-agent/) — running the agent that consumes the adapted tools.
