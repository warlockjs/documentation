...
`ai.speech()` is the audio-output counterpart to [`ai.image()`](/v/latest/ai/modalities/generate-images/). Text-in / audio-out, wrapped in the **same** uniform result contract every AI verb returns — so a synthesized voicemail slots into your cost dashboards and observability traces exactly like an agent run.

This is audio **output** (TTS). For audio **input** — transcribing a WhatsApp voice note or a meeting recording — see [Transcribe audio](/v/latest/ai/modalities/transcribe-audio/).
...
:::note
No adapter validates the model id locally. `openai.speech({ name })` forwards the id to the provider exactly as given, so a non-TTS id (`openai.speech({ name: "gpt-4o" })`) constructs fine and fails at the provider — as a typed provider error on `result.error`, never as a local throw at construction.
:::
...
- [Transcribe audio](/v/latest/ai/modalities/transcribe-audio/) — the inverse verb (`ai.transcribe`), audio → text.
- [Generate images](/v/latest/ai/modalities/generate-images/) — the sibling image-output verb (`ai.image`).
- [Run agent](/v/latest/ai/agents/run-agent/) — the chat/tool-calling verb.
- [Handle errors](/v/latest/ai/reliability/handle-errors/) — the typed `AIError` taxonomy on `result.error`.
