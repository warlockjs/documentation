...
- [Generate images](/v/latest/ai/modalities/generate-images/) — still-image output (`ai.image`), the synchronous sibling `ai.video` mirrors.
- [Run orchestrator](/v/latest/ai/orchestration/run-orchestrator/) — the other session-shaped primitive `ai.realtime` resembles.
- [Log AI calls](/v/latest/ai/observability/log-ai-calls/) — the `observe` seam both verbs route their reports through.
- [Handle errors](/v/latest/ai/reliability/handle-errors/) — how the typed `AIError` on `result.error` and the realtime `error` event surface.
