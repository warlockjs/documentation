...
`ai.image()` is the output counterpart to [`ai.agent`](/v/latest/ai/agents/run-agent/) for the image modality. Prompt-in / images-out, wrapped in the **same** uniform result contract every AI verb returns — so an image generation slots into your cost dashboards and observability traces exactly like an agent run.

This is image **output** (generation). For image or PDF **input** to a chat agent (vision), see [Run agent](/v/latest/ai/agents/run-agent/).
...
- [Generate speech](/v/latest/ai/modalities/generate-speech/) — the audio-output verb (`ai.speech`).
- [Transcribe audio](/v/latest/ai/modalities/transcribe-audio/) — the audio-input verb (`ai.transcribe`).
- [Run agent](/v/latest/ai/agents/run-agent/) — the chat/tool-calling verb, and image/vision **input**.
