...
### Validate against your own criteria

The judge grades against a built-in quality rubric by default. Pass **`criteria`** — a string or a list of short rules — to grade the prompt against **your own** rules instead. The judge's `score` / `issues` then reflect your criteria (a failed rule is named in `issues`); it's still advisory and never flips `ok`.
...
The criteria are folded into the `judgeCache` key, so re-validating the same prompt with different rules re-runs the judge instead of returning a stale verdict.
...
`systemPrompt().validate(options?)` is the per-builder sugar — exactly `ai.prompts.validate(this, options)`. See [Write system prompts](/v/latest/ai/prompts/write-system-prompts/#validate-this-prompt).
...
Pair `validate()` with [`agent.eval`](/v/latest/ai/testing/evaluation-and-datasets/) for a two-layer quality gate: lint the prompt text, then measure the resulting behaviour against a dataset.

## Related

- [Write system prompts](/v/latest/ai/prompts/write-system-prompts/) — the `SystemPrompt` builder, `.meta()` identity, and registry-aware `.merge()`.
- [Refine prompts](/v/latest/ai/prompts/refine-prompts/) — `systemPrompt().refined({ model, criteria, store })`, the prompt compiler; register its output as a next version to `diff` original vs refined.
- [Evaluation and datasets](/v/latest/ai/testing/evaluation-and-datasets/) — grade the *behaviour* a prompt produces, not just the text.
- [Run agent](/v/latest/ai/agents/run-agent/) — where a resolved prompt is consumed.
