...
This is the dashboard's **one write-capable route** — everything else is `GET`-only by design (see [Wire Panoptic](/v/latest/ai/recipes/observability-wire-panoptic/)). It's off unless you configure it.
...
- [Observability — wire Panoptic](/v/latest/ai/recipes/observability-wire-panoptic/) — attach the subscriber and the store the dashboard reads from.
- [Observability — trace cost dashboard](/v/latest/ai/recipes/observability-trace-cost-dashboard/) — the read-only query layer this write route sits alongside.
