...
- [Guardrails](/v/latest/ai/reliability/guardrails/) — runtime content detectors (`ai.guardrail`); not the same as this key-driven utility.
- [Handle errors](/v/latest/ai/reliability/handle-errors/) — the typed `AIError` hierarchy whose `cause` `redactError` sanitizes.
- [Record / replay testing](/v/latest/ai/testing/record-replay-testing/) — VCR cassettes redact recorded requests with this policy.
- [What Panoptic traces](/v/latest/ai/observability/what-panoptic-traces/) — captured content runs through the same redaction.
- [Log AI calls](/v/latest/ai/observability/log-ai-calls/) — channel-level redaction for logs (a separate `@warlock.js/logger` feature).
- [API reference](/v/latest/ai/reference/api/) — full export index.
