...
- [Human in the loop](/v/latest/ai/reliability/human-in-the-loop/) — the OTHER resume: `ai.human.resume` of a gated tool call, not a crash.
- [Persist AI data](/v/latest/ai/reliability/persist-ai-data/) — the supervisor / workflow snapshot resume (the sibling `iterate`-style durability), the `ai.snapshot.{memory,pg,redis}` factories, the `SnapshotStore` contract, and the never-auto-migrated `schema()`.
- [Handle errors](/v/latest/ai/reliability/handle-errors/) — the typed `AgentDriftError` / `PlannerDriftError` / `AgentExecutionError` / `PlannerFailedError` and how `result.error` surfaces a failed run.
