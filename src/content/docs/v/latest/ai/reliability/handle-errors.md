...
- [Run agent](/v/latest/ai/agents/run-agent/) — `AgentResult.error`.
- [Run workflow](/v/latest/ai/orchestration/run-workflow/) — `WorkflowError` subclasses.
- [Run orchestrator](/v/latest/ai/orchestration/run-orchestrator/) — `OrchestratorDriftError`, `OrchestratorConfigError`.
- [Planner](/v/latest/ai/architecture-concepts/planner/) — `PlannerPlanInvalidError`, `PlannerCancelledError`.
- [Define tools](/v/latest/ai/tools/define-tools/) — `ToolExecutionError` wrapping.
- [Log AI calls](/v/latest/ai/observability/log-ai-calls/) — error logging.
