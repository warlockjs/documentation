...
It is **middleware on the agent**, so it composes with everything else — guardrails, budget, semantic cache. See [Attach middleware](/v/latest/ai/reliability/attach-middleware/).
...
- [Attach middleware](/v/latest/ai/reliability/attach-middleware/) — how approval composes with budget, guardrail, cache; ordering invariants.
- [Guardrails](/v/latest/ai/reliability/guardrails/) — automated content gating (`escalation.onBlock` can hand off to a human-review hook).
- [Run agent](/v/latest/ai/agents/run-agent/) — the agent the gate wraps; `result.error` handling.
- [Define tools](/v/latest/ai/tools/define-tools/) — the tools a policy gates.
