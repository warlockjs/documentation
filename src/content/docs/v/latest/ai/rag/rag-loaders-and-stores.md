...
Two feature groups bracket [`ai.rag()`](/v/latest/ai/rag/run-rag/): **loaders** turn a source — a string, raw HTML, a URL, or PDF bytes — into the exact `RagDocument[]` shape that `kb.index()` consumes, and **stores** are the swappable backends that hold the embeddings. Both live on the `ai.rag.*` namespace, present the moment `@warlock.js/ai` is imported — no side-effect import, no module augmentation.
...
- [Run RAG](/v/latest/ai/rag/run-rag/) — the chunk → embed → retrieve → rerank → cite pipeline that **consumes** these loaders and stores.
- [Embed text](/v/latest/ai/rag/embed-text/) — the `sdk.embedder` primitive whose `dimensions` must match the store's `vector(N)` width.
- [Persist AI data](/v/latest/ai/reliability/persist-ai-data/) — the semantic-cache vector store, wired from the same `@warlock.js/cache` drivers.
- [Outbound policy](/v/latest/ai/reliability/outbound-policy/) — the `OutboundPolicy` and `guardedFetch` that make `loadWeb` SSRF-safe.
