...
:::caution
`every(value, unit)` throws at definition time if `value` is `0`, negative, `NaN`, or `Infinity`. This guards against the most common mistake — a misconfigured "every N" that would otherwise spin the scheduler.
:::
...
:::caution
`at()` validates the format and the ranges. The hour must be `0–23`, minute `0–59`, second `0–59`. Bad input (`"24:00"`, `"23:60"`, `"foo"`) throws at definition time.
:::
