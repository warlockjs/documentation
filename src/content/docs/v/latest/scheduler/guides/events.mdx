...
Fires when a tick's scheduled run cannot proceed because the job's callback is already executing. Typically this happens when the job was kicked off outside the scheduler's loop — a manual `job.run()`, a boot-time recovery sweep, another scheduler instance pointing at the same job — and the in-flight run is still going when the next scheduled tick arrives. See [Overlap Prevention](./overlap-prevention) for the scenarios where this comes up.
