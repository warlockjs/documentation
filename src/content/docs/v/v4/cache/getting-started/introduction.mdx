...
- **8 interchangeable drivers** — Redis, Memory, Memory Extended, LRU, File, Null, Postgres (with optional pgvector), and Mock for [tests](/v/v4/cache/testing/).
...
- **Similarity retrieval** — store embeddings alongside entries, query by meaning ([what's similarity?](/v/v4/cache/similarity/)).
- **Stampede prevention** — when 1000 concurrent requests all miss the same key, only one hits the source of truth ([what's a stampede?](/v/v4/cache/stampede-prevention/)).
- **Stale-while-revalidate** — return cached data instantly while a background refresh runs, so slow upstreams never make fast endpoints slow ([SWR guide](/v/v4/cache/swr/)).
- **Event system** — observe every cache operation.
- **Built-in metrics** — `cache.metrics()` returns hit rate, latency percentiles, per-driver breakdowns ([metrics guide](/v/v4/cache/metrics/)).
...
| Method | Description |
|--------|-------------|
| `set(key, value, ttlOrOptions?)` | Store a value with optional TTL or [rich options](/v/v4/cache/set-options/) |
...
- [Set Options](/v/v4/cache/set-options/) — rich third-arg object: `ttl`, `expiresAt`, `tags`, `onConflict`, per-call `driver`
- [Update & Merge](/v/v4/cache/update-merge/) — atomic read-modify-write for structured values
- [Cache Lists](/v/v4/cache/lists/) — queue / stack / recent-N buffer sub-API
- [Cache Tags](/v/v4/cache/tags/) — group related entries and invalidate them together
- [Atomic Operations](/v/v4/cache/atomic-operations/) — increment, decrement, and conditional writes without race conditions
- [Stampede Prevention](/v/v4/cache/stampede-prevention/) — `remember()` shares one concurrent regeneration per key
- [Events](/v/v4/cache/events/) — observe every cache operation with 9 event types
