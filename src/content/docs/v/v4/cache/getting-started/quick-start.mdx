...
Three escalating tools: `set`/`get` for direct control, [`cached()`](/v/v4/cache/cached/)
when freshness can lag a little and you call the same function from many places,
and [`swr()`](/v/v4/cache/swr/) when a slow upstream must never make a fast
...
Full reference in [Set Options](/v/v4/cache/set-options/). Skip this section on your first read if you just want to cache some data.
...
Compound forms like `"1h30m"` are not supported — use `"90m"` or the numeric equivalent. Duration strings work everywhere a TTL is accepted: `set()`, `remember()`, driver options, and inside [`CacheSetOptions`](/v/v4/cache/set-options/).
...
- [Configuration](/v/v4/cache/configurations/) — set up Redis or other drivers
- [Cache Manager](/v/v4/cache/cache-manager/) — complete method reference
- [Set Options](/v/v4/cache/set-options/) — `ttl`, `expiresAt`, `tags`, `onConflict`, per-call `driver` override
- [Stampede Prevention](/v/v4/cache/stampede-prevention/) — why `remember()` is special
- [Cache Tags](/v/v4/cache/tags/), [Events](/v/v4/cache/events/), and more advanced features
