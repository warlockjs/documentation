...
:::note
Namespace strings are always treated as data, not as glob/path input, even when derived from user-controlled values (a request param used to build `orders.${userId}`). The [file driver](/v/v4/cache/file/#key-safety) percent-encodes and path-contains every key/namespace; the [redis driver](/v/v4/cache/redis/#namespace-removal) escapes glob metacharacters and walks matches with `SCAN` rather than the blocking `KEYS` command.
:::
...
Both namespaces and [cache tags](/v/v4/cache/tags/) help organize cache, but they serve different purposes:
...
- [Cache Manager](/v/v4/cache/cache-manager/) - Basic operations and driver management
- [Cache Tags](/v/v4/cache/tags/) - Alternative grouping method with tags
- [Cache Keys Best Practices](/v/v4/cache/cache-manager/#cache-keys) - Key naming conventions
