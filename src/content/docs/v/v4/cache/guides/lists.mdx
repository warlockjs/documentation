...
Single-process, single-writer usage (typical test / script / job-runner) is fine. For concurrent writers across processes, gate writes with a distributed lock via [`onConflict: "create"`](/v/v4/cache/set-options/#distributed-locking-recipe), or wait for the Redis-native override in v2.1.
:::
...
- [Set Options](/v/v4/cache/set-options/) — distributed-lock recipe for concurrent-safe list writes
- [Cache Manager](/v/v4/cache/cache-manager/) — full API surface
- [Events](/v/v4/cache/events/) — list ops emit `set` / `removed` just like regular `set`/`remove`
