...
**Until it lands:** if cross-process safety matters today, wrap the update in a distributed lock using [`onConflict: "create"`](/v/v4/cache/set-options/#distributed-locking-recipe). Example:
...
The file driver is deliberately unsupported — it has no file-lock primitive today, so concurrent updates would clobber each other silently. Throwing is safer than lying. See [Errors](/v/v4/cache/errors/) for the `CacheUnsupportedError` pattern.

## When to reach for what

| Task | Use |
|------|-----|
| "Add 1 to this counter" | [`increment()`](/v/v4/cache/atomic-operations/#increment-and-decrement) — Redis-native `INCRBY` |
| "Change one field on a cached object" | `merge()` |
| "Read-decide-maybe-write, possibly remove" | `update()` |
| "Only set if missing" | [`set(k, v, { onConflict: "create" })`](/v/v4/cache/set-options/#conditional-writes--onconflict) |
| "Only set if already exists" | [`set(k, v, { onConflict: "update" })`](/v/v4/cache/set-options/#conditional-writes--onconflict) |
| "Read-then-delete atomically" | [`pull()`](/v/v4/cache/cache-manager/) |
...
- [Set Options](/v/v4/cache/set-options/) — `onConflict` for conditional writes
- [Atomic Operations](/v/v4/cache/atomic-operations/) — counters and conditional-write patterns
- [Stampede Prevention](/v/v4/cache/stampede-prevention/) — `remember()` for cache-aside lookups
- [Errors](/v/v4/cache/errors/) — `CacheUnsupportedError`
