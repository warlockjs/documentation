...
For dev, start with `memory`. For production, switch to [`pg`](/v/v4/cache/pg/) with the `vector` config block — same code, real index.
...
For real production workloads, use the [`pg` driver](/v/v4/cache/pg/) with `vector` config — pgvector's HNSW index is sub-linear and battle-tested.

## See also

- [`pg` driver](/v/v4/cache/pg/) — Postgres + pgvector setup
- [Set Options](/v/v4/cache/set-options/) — TTL, tags, conflict policy details
- [Tags](/v/v4/cache/tags/) — building tag-narrowed knowledge bases
