...
Uses [`remember()`](/v/v4/cache/stampede-prevention/) under the hood — inherits stampede protection for free.

## When to use `cached` vs `remember`

| Task | Use |
|------|-----|
| One-shot "get this value or compute it" | [`cache.remember()`](/v/v4/cache/stampede-prevention/) |
...
- [Stampede Prevention](/v/v4/cache/stampede-prevention/) — `remember()`, the primitive under the hood
- [Set Options](/v/v4/cache/set-options/) — the shape of the options object `cached` forwards to `set()`
- [Cache Tags](/v/v4/cache/tags/) — tag-based invalidation
- [Errors](/v/v4/cache/errors/) — `CacheConfigurationError` and when it fires
