...
Both share the same tag index — invalidate them with `cache.tags([...]).invalidate()` regardless of which shape you used at write time. See [Set Options](/v/v4/cache/set-options/#inline-tags).
:::
...
See [Best Practices](/v/v4/cache/best-practices/) for more tag organization strategies.
