...
Don't use `lock` for:
- **Memoizing a function's result** — use [`cached()`](/v/v4/cache/cached/) or [`remember()`](/v/v4/cache/stampede-prevention/) instead.
...
- **Don't use `lock()` for memoization** — use [`cached()`](/v/v4/cache/cached/) or [`remember()`](/v/v4/cache/stampede-prevention/).
...
- [Set Options — Conditional writes](/v/v4/cache/set-options/#conditional-writes--onconflict) — the `onConflict: "create"` primitive `lock()` is built on
- [Stampede Prevention](/v/v4/cache/stampede-prevention/) — `remember()` for cache-aside patterns (different use case)
- [`cached()`](/v/v4/cache/cached/) — HOF memoization (different use case)
- [Atomic Operations](/v/v4/cache/atomic-operations/) — the full atomic toolkit
