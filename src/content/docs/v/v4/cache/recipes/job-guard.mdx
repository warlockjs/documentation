...
`lock()` returns a [`LockOutcome`](/v/v4/cache/lock/) discriminated union:
...
On the [`redis`](/v/v4/cache/redis/) driver the lock is built on native
...
- [Distributed Locks — cache.lock()](/v/v4/cache/lock/) — the full lock API
- [Set Options](/v/v4/cache/set-options/#conditional-writes--onconflict) — the `onConflict: "create"` primitive locks are built on
- [Redis Cache Driver](/v/v4/cache/redis/) — native `SET NX` locking
