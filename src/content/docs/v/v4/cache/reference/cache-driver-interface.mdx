...
slug: v/v4/cache/cache-driver-interface
---
...
Store a value in cache. The third argument accepts three shapes — a positional TTL (number or duration string) for the common case, or a rich [`CacheSetOptions`](/v/v4/cache/set-options/) object.
...
See [Set Options](/v/v4/cache/set-options/) for the full option reference.
...
See [Stampede Prevention](/v/v4/cache/stampede-prevention/) for details.
...
Throws `CacheUnsupportedError` on the file driver. See [Update & Merge](/v/v4/cache/update-merge/).
...
Obtain a list accessor bound to the key. Returns a [`CacheListAccessor<T>`](/v/v4/cache/lists/) for queue/stack/recent-N workflows.
...
Drivers that don't index vectors throw `CacheUnsupportedError`. See [Similarity Retrieval](/v/v4/cache/similarity/) for the full API and capability matrix.
...
See [Event System](/v/v4/cache/events/) for the full event catalog.
...
See [Set Options — Conditional writes](/v/v4/cache/set-options/#conditional-writes--onconflict).
...
See [Similarity Retrieval](/v/v4/cache/similarity/).
...
See [Cache Lists](/v/v4/cache/lists/).
...
See [Errors](/v/v4/cache/errors/) for the full catalog.
...
See [Event System](/v/v4/cache/events/).

## Implementing a Custom Driver

- Extend [BaseCacheDriver](/v/v4/cache/base-cache-driver/) to reuse key parsing, logging, TTL handling, event emission, and default implementations for `has`, `remember`, `pull`, `forever`, `increment`, `decrement`, `many`, `setMany`, `tags`, `update`, `merge`, and `list`.
...
See [Make Your Own Cache Driver](/v/v4/cache/make-your-own-cache-driver/) for a complete end-to-end example.
