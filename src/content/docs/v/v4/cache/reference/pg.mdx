...
- You're building [semantic caching](/v/v4/cache/similarity/), RAG retrieval, or "find similar items" features and want a real ANN index — not the brute-force memory scan.
...
Both patterns produce the same loaded driver. See [Cache Manager → Runtime driver options](/v/v4/cache/cache-manager/#runtime-driver-options) for merge precedence and reload semantics.
...
The `stale_at` column powers [stale-while-revalidate](/v/v4/cache/swr/) — entries written via `cache.swr(...)` populate it so the driver can branch on freshness on every read. Plain `set()` calls leave the column null (treated as always-fresh by SWR).
...
See [Similarity Retrieval](/v/v4/cache/similarity/) for the full `similar()` API.
...
- [Similarity Retrieval](/v/v4/cache/similarity/) — the `similar()` API in depth
- [Configurations](/v/v4/cache/configurations/) — driver setup at the package level
- [Set Options](/v/v4/cache/set-options/) — TTL, tags, conflict policy
- [Tags](/v/v4/cache/tags/) — group invalidation
