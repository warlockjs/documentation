...
## Troubleshooting
- **Data disappears after restart**: Expected. Use [File](/v/v4/cache/file/) or [Redis](/v/v4/cache/redis/) for persistence.
...
✅ **Supported (brute force).** `set({ vector })` indexes the entry; `similar()` does an O(N) cosine scan over live nodes. LRU eviction also drops the vector. Dev-only at scale — see [Similarity Retrieval](/v/v4/cache/similarity/).
