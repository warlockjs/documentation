...
slug: v/v4/cache/base-cache-driver
---
...
Normalize a cache key using internal [parseCacheKey](/v/v4/cache/utils/#parse-cache-key) utility.
...
Internally composed of three pure helpers exported from [`utils`](/v/v4/cache/utils/) — useful when writing custom drivers that need the same normalization without inheriting from `BaseCacheDriver`:

- [`normalizeToOptions(input)`](/v/v4/cache/utils/#normalize-to-options) — coerce the polymorphic 3rd `set` arg into a `CacheSetOptions` shape.
- [`resolveTtl(ttl, expiresAt, fallback)`](/v/v4/cache/utils/#resolve-ttl) — apply caller > driver-default precedence + mutual-exclusion check.
- [`expiresAtToTtl(expiresAt)`](/v/v4/cache/utils/#expiresat-to-ttl) — Date/epoch → relative seconds.
...
The base class also ships a default `similar()` that throws `CacheUnsupportedError` — drivers that support similarity override it with a real impl. See [Make Your Own Driver](/v/v4/cache/make-your-own-cache-driver/) for the full extension surface.

Throws [`CacheConfigurationError`](/v/v4/cache/errors/#cacheconfigurationerror) on:
...
See [Cache Driver Interface](/v/v4/cache/cache-driver-interface/) for the complete interface definition.
