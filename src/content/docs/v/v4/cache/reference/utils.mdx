...
[`CacheConfigurationError`](/v/v4/cache/errors/#cacheconfigurationerror)
...
a uniform [`CacheSetOptions`](/v/v4/cache/set-options/) shape, so callers
...
[`cache.similar()`](/v/v4/cache/similarity/) uses it under the hood for
...
- [Set Options](/v/v4/cache/set-options/) — the `CacheSetOptions` shape these helpers normalize
- [Similarity Retrieval](/v/v4/cache/similarity/) — `cache.similar()`, which uses `cosineSimilarity`
- [Make Your Own Cache Driver](/v/v4/cache/make-your-own-cache-driver/) — where these helpers earn their keep
- [Errors](/v/v4/cache/errors/) — `CacheConfigurationError` and friends
