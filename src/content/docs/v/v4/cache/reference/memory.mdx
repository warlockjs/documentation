...
- Memory growth: Without `maxSize`, cache can grow indefinitely (use `maxSize` or [LRU Memory Cache Driver](/v/v4/cache/lru-memory/) for automatic eviction)

## Alternatives
- [Memory Extended Cache Driver](/v/v4/cache/memory-extended/): Sliding expiration (TTL resets on access)
- [LRU Memory Cache Driver](/v/v4/cache/lru-memory/): Evicts least recently used items when full
...
:::tip
For dedicated LRU caching with more control, consider using the [LRU Memory Cache Driver](/v/v4/cache/lru-memory/) instead.
:::
...
## Troubleshooting
- **Data disappears after restart**: This is expected. Use [File](/v/v4/cache/file/) or [Redis](/v/v4/cache/redis/) drivers for persistence.
- **Cache not shared between processes**: Use [Redis](/v/v4/cache/redis/) for distributed cache.
- **Memory usage growing**: Set `maxSize` option or use [LRU Memory Cache Driver](/v/v4/cache/lru-memory/) for automatic eviction.
- **Items being evicted unexpectedly**: Check if `maxSize` is set and if cache is reaching the limit.

:::note
:::

✅ **Supported (brute force).** `set({ vector })` indexes the entry; `similar()` does an O(N) cosine scan. Suitable for development and small datasets — see [Similarity Retrieval](/v/v4/cache/similarity/). Past a few thousand entries, switch to the [`pg` driver](/v/v4/cache/pg/) with pgvector.
