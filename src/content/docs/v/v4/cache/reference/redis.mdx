...
## Alternatives
- [File Cache Driver](/v/v4/cache/file/): Simpler, persistent, but not distributed
- [Memory Cache Driver](/v/v4/cache/memory/): Fastest, but not shared or persistent

## Configuration

Configure the Redis driver in your [cache configurations](/v/v4/cache/configurations/):
...
See [Set Options — Conditional writes](/v/v4/cache/set-options/#conditional-writes--onconflict) for the full reference and the return shape.
...
See [Atomic Operations](/v/v4/cache/atomic-operations/) for comprehensive documentation and examples.
...
❌ **Not supported today.** `cache.set(k, v, { vector })` and `cache.similar(...)` throw `CacheUnsupportedError`. RediSearch-backed similarity is on the [backlog](/v/v4/cache/similarity/#capability-matrix). Use the [`pg` driver](/v/v4/cache/pg/) with `vector` config for production similarity, or a memory driver for development.
