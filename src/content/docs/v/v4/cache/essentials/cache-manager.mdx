...
:::danger
Always initialize at application startup. Using cache methods before `init()` throws [`CacheDriverNotInitializedError`](/v/v4/cache/errors/#cachedrivernotinitializederror).
:::
...
See [Set Options](/v/v4/cache/set-options/) for the full options-object reference — `ttl`, `expiresAt`, `tags`, `onConflict`, and per-call `driver` overrides.
...
:::tip
Always prefer `remember()` for expensive operations (database queries, API calls, computations). See [Cache Stampede Prevention](/v/v4/cache/stampede-prevention/).
:::
...
See [Update & Merge](/v/v4/cache/update-merge/) for the callback contract, TTL preservation rules, and driver support matrix.
...
See [Cache Lists](/v/v4/cache/lists/) for the full accessor API.
...
See [Similarity Retrieval](/v/v4/cache/similarity/) for the full API and capability matrix — only memory drivers and `pg` (with `vector` config) support it; others throw `CacheUnsupportedError`.
...
`currentDriver` is `undefined` until `init()` or `use()` completes. Every data op guards with an init check, so calling `get` / `set` before init throws [`CacheDriverNotInitializedError`](/v/v4/cache/errors/#cachedrivernotinitializederror).
...
Once a driver is loaded, options are frozen on the instance. Calling `load` / `driver` / `use` a second time with **new** options throws [`CacheConfigurationError`](/v/v4/cache/errors/#cacheconfigurationerror) — no silent overwrites:
...
Set it in your [configuration](/v/v4/cache/configurations/) or update at runtime with `setOptions()`.

## Namespaces

Namespaces help organize cache keys hierarchically using dot notation. See [Namespaces](/v/v4/cache/namespaces/) for a complete guide, including `cache.namespace(prefix, options?)` for scoped handles.
...
- [Set Options](/v/v4/cache/set-options/) — rich `CacheSetOptions` object: `ttl`, `expiresAt`, `tags`, `onConflict`, `driver`
- [Update & Merge](/v/v4/cache/update-merge/) — atomic read-modify-write
- [Cache Lists](/v/v4/cache/lists/) — queue / stack / recent-N buffer accessor
- [Cache Tags](/v/v4/cache/tags/) — group and invalidate related entries
- [Atomic Operations](/v/v4/cache/atomic-operations/) — `increment`, `decrement`, conditional writes
- [Bulk Operations](/v/v4/cache/bulk-operations/) — `many()` and `setMany()` batching
- [Stampede Prevention](/v/v4/cache/stampede-prevention/) — `remember()` and distributed-lock recipes
- [Event System](/v/v4/cache/events/) — observe every cache operation
...
:::tip Recommended
Dot notation makes keys predictable, readable, and easier to debug. If you need to generate keys from complex data, see [`parseCacheKey`](/v/v4/cache/utils/#parse-cache-key).
:::
...
:::info
- **Always call `init()` before using cache operations** — operations throw [`CacheDriverNotInitializedError`](/v/v4/cache/errors/#cachedrivernotinitializederror) otherwise.
...
- [Cache Configurations](/v/v4/cache/configurations/) — configure drivers and options
- [Set Options](/v/v4/cache/set-options/) — the headline v2 feature
- [Cache Driver Interface](/v/v4/cache/cache-driver-interface/) — complete method reference
- [Errors](/v/v4/cache/errors/) — error classes and how to react
- [Best Practices](/v/v4/cache/best-practices/) — production-ready caching strategies
