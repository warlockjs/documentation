...
See [Duration Strings](/v/v4/cache/quick-start/#duration-strings) for the full grammar of accepted string forms.
...
| `vector` | `number[]` | Embedding indexed alongside the entry for [similarity retrieval](/v/v4/cache/similarity/). Drivers without similarity support throw `CacheUnsupportedError`. |
...
See [Cache Tags](/v/v4/cache/tags/) for the full invalidation workflow.
...
See [Stampede Prevention](/v/v4/cache/stampede-prevention/) for the single-node variant using `remember()`.
...
- [Atomic Operations](/v/v4/cache/atomic-operations/) — counters, `update()`, `merge()`
