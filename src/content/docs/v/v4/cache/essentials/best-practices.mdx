...
slug: v/v4/cache/best-practices
---
...
See [Update & Merge](/v/v4/cache/update-merge/) and [Set Options](/v/v4/cache/set-options/) for details.
...
`similar()` looks up entries by vector similarity instead of exact key match. Useful for [semantic caching](/v/v4/cache/similarity/), RAG, and "find similar" features. A few rules of thumb:
...
- **`CacheUnsupportedError` from `similar()`?** The current driver doesn't index vectors. Check the [capability matrix](/v/v4/cache/similarity/#capability-matrix).
