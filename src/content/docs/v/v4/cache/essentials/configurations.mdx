...
slug: v/v4/cache/configurations
---
...
At the driver-config level, the `ttl` key accepts **either** a number of seconds **or** a human-readable duration string (`"1h"`, `"30m"`, `"7d"`). The rich [`CacheSetOptions`](/v/v4/cache/set-options/) object is only meaningful at the call site, not in global config.
:::

After defining your configuration, you'll need to set it and initialize the cache manager. See [Cache Manager](/v/v4/cache/cache-manager/) for details on using `setCacheConfigurations()` and `init()`.
...
Once configured, you can switch between drivers at runtime using the cache manager. See [Switching Drivers](/v/v4/cache/cache-manager/#set-default-driver) for details.
...
When using custom drivers, you must specify the driver name(s) as a generic parameter to `CacheConfigurations`. See [Make Your Own Cache Driver](/v/v4/cache/make-your-own-cache-driver/#registering-your-driver) for more details.
:::

Learn how to [create custom drivers](/v/v4/cache/make-your-own-cache-driver/).

## Driver Options

Each driver has its own configuration options. See the individual driver documentation for detailed options:

- [Redis Cache Driver](/v/v4/cache/redis/#options) - Redis-specific options
- [File Cache Driver](/v/v4/cache/file/#options) - File-specific options
- [Memory Cache Driver](/v/v4/cache/memory/#options) - Memory-specific options
- [Memory Extended Cache Driver](/v/v4/cache/memory-extended/#options) - Memory Extended-specific options
- [LRU Memory Cache Driver](/v/v4/cache/lru-memory/#options) - LRU-specific options
- [Null Cache Driver](/v/v4/cache/null/#options) - Null-specific options
...
Runtime options merge **over** static ones per-key (runtime wins). See [Cache Manager → Runtime driver options](/v/v4/cache/cache-manager/#runtime-driver-options) for the full pattern.
