...
- File driver: a key/namespace resolves outside the configured cache root even after percent-encoding — `"Cache key resolves outside the cache directory: ..."`. Last line of defense against path traversal; shouldn't happen from normal keys. See [File Cache Driver — Key Safety](/v/v4/cache/file/#key-safety).
...
- `cache.set(k, v, { vector })` and `cache.similar(...)` on drivers that don't index vectors (file, redis until RediSearch lands, pg without the `vector` config block) — see [Similarity Retrieval](/v/v4/cache/similarity/)
...
Reserved for driver connection failures. Not thrown by any built-in driver today — the Redis driver currently logs the error and emits an `"error"` event on the manager bus instead. See [Events](/v/v4/cache/events/) for how to subscribe.
...
- [Cache Manager](/v/v4/cache/cache-manager/) — `init()` and setup
- [Set Options](/v/v4/cache/set-options/) — validation rules for options object
- [Update & Merge](/v/v4/cache/update-merge/) — `CacheUnsupportedError` on the file driver
- [Events](/v/v4/cache/events/) — connection errors surface here
