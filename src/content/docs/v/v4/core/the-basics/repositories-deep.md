...
See [Events and hooks](/v/v4/cascade/architecture-concepts/events-and-hooks/) for the full model event surface (`creating`/`created`, `updating`/`updated`, `saving`/`saved`, `deleting`/`deleted`, `fetching`, and so on).
...
- **[Events and hooks](/v/v4/cascade/architecture-concepts/events-and-hooks/)** — Cascade model lifecycle events the repository's invalidation hooks into.
