...
These also fire the model's lifecycle events (`creating` → `created`, `updating` → `updated`, `deleting` → `deleted`), which is how the cache invalidation hook above stays in sync. See **[Events and hooks](/v/v4/cascade/architecture-concepts/events-and-hooks/)** in the Cascade docs for the full event surface.
...
For the full model event surface, see **[Events and hooks](/v/v4/cascade/architecture-concepts/events-and-hooks/)** in the Cascade docs.
...
- **[Events and hooks](/v/v4/cascade/architecture-concepts/events-and-hooks/)** — Cascade model lifecycle events the repository's cache invalidation hooks into.
