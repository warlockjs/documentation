...
This recipe uses Cascade's `"soft"` delete strategy. For the `"trash"` strategy (move to a separate table) and the full strategy comparison, see [delete strategies](/v/v4/cascade/digging-deeper/delete-strategies/).
...
For the full global-scope surface (`withoutGlobalScope`, `withoutGlobalScopes`, local scopes) see the [scopes guide](/v/v4/cascade/digging-deeper/scopes/).
...
Now every soft-delete, restore, and permanent-delete leaves an audit trail with the strategy and the actor. The full audit pattern lives in the [audit-trail recipe](/v/v4/cascade/recipes/audit-trail/) — it's worth a read.
...
- **Cascade's full delete-strategy story (`permanent` / `soft` / `trash`):** [Delete strategies](/v/v4/cascade/digging-deeper/delete-strategies/)
- **Auditing every delete and restore:** [Audit trail recipe](/v/v4/cascade/recipes/audit-trail/)
- **Hiding soft-deleted rows via global scopes:** [Scopes guide](/v/v4/cascade/digging-deeper/scopes/)
