...
- **`request.ip` honors `X-Forwarded-For` only if `http.trustProxy` is set.** Behind a load balancer without it, every IP looks like your LB's. Set `http.trustProxy` to the narrowest shape your topology allows — a hop count or a trusted-proxy list, not bare `true` — otherwise your "per-IP" limit can become "per-client-chosen-IP" instead of "per-load-balancer". See [Security → `trustProxy` shapes](../digging-deeper/security.md#trustproxy-shapes).
