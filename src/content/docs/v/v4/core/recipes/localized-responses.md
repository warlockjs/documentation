...
The validation schema and migration use Cascade's localized helpers — `v.localized(v.string())` for the schema and the column definition. Refer to the [validation guide](../the-basics/validation.md) for the schema side and [migrations guide](/v/v4/cascade/the-basics/migrations/) for the column definition.
