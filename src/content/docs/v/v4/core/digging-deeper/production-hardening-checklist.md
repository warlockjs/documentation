...
- [ ] **Decide who you trust for `X-Forwarded-For`.** **Since 4.13.0 `http.trustProxy` defaults to `false`**, so `request.detectIp()`, `middleware.ipFilter()` and IP-scoped rate limiting use the socket address. **Since 4.15.0, prefer a hop count or a trusted-proxy CIDR list over bare `true`** — `true` trusts the whole `X-Forwarded-For` chain, including whatever the client itself prepended if your edge only appends (the common nginx/ALB/CDN case); a hop count (`2`) or a proxy list (`"10.0.0.0/8"`) walks only past your own known proxies. Setting `true` without a proxy that overwrites the header lets any client choose its own IP and makes per-IP rate limits and IP allowlists bypassable. `X-Real-IP` is honoured only under `trustProxy: true` — a bounded `trustProxy` reads `X-Forwarded-For` instead. See [Security → `trustProxy` shapes](./security.md#trustproxy-shapes) and [Known Limitations → transport & proxy trust](./known-limitations.md).
