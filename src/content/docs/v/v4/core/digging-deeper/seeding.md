...
The manager doesn't act on `batchSize` itself — it only hands the value through; the chunking happens inside `createMany`. See the Cascade [Bulk inserts guide](/v/v4/cascade/digging-deeper/bulk-inserts/) for what `createMany` does with it.
...
- **[Bulk inserts](/v/v4/cascade/digging-deeper/bulk-inserts/)** — what `Model.createMany(rows, { batchSize })` does with the relayed `batchSize`.
