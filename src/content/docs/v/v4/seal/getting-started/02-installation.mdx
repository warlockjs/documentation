...
Inside a Warlock app, the same schema slots into any `StandardSchemaV1<T>`-typed slot — request validation, AI tool input, Cascade `Model.schema` — because every seal schema implements Standard Schema. New to Warlock? Start with [core / getting-started](/v/v4/core/getting-started/01-introduction/).
