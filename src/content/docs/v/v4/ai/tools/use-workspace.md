...
- [Use AI tools](/v/v4/ai/tools/use-ai-tools/) — the network/utility tool belt (web search, fetch, HTTP, calculator, date-time).
- [Connect MCP](/v/v4/ai/tools/connect-mcp/) — `ai.mcp.serve({ tools: () => ws.tools.all() }, ...)` publishes a workspace AS an MCP server.
- [Run supervisor](/v/v4/ai/orchestration/run-supervisor/) — coordinating the members that share one workspace.
- [Run agent](/v/v4/ai/agents/run-agent/) — the agent loop, `maxTrips`, and the run report.
