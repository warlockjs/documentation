...
- [Connect MCP](/v/v4/ai/tools/connect-mcp/) — `ai.mcp(...)` consumes an external MCP server's tools, and `ai.mcp.serve(...)` exposes these tools AS an MCP server. Both ship in `@warlock.js/ai-tools`.
- [Use a workspace](/v/v4/ai/tools/use-workspace/) — the filesystem + shell tool belt (jailed `read_file` / `edit_file` / `run_shell` / …).
- [Define tools](/v/v4/ai/tools/define-tools/) — the `tool()` / `ToolContract` seam every tool here is built on, for when you need a custom one.
- [Run agent](/v/v4/ai/agents/run-agent/) — running the agent that consumes these tools.
