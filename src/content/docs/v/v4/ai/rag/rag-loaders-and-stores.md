...
Two feature groups bracket [`ai.rag()`](/v/v4/ai/rag/run-rag/): **loaders** turn a source — a string, raw HTML, a URL, or PDF bytes — into the exact `RagDocument[]` shape that `kb.index()` consumes, and **stores** are the swappable backends that hold the embeddings. Both live on the `ai.rag.*` namespace, present the moment `@warlock.js/ai` is imported — no side-effect import, no module augmentation.
...
- [Run RAG](/v/v4/ai/rag/run-rag/) — the chunk → embed → retrieve → rerank → cite pipeline that **consumes** these loaders and stores.
- [Embed text](/v/v4/ai/rag/embed-text/) — the `sdk.embedder` primitive whose `dimensions` must match the store's `vector(N)` width.
- [Persist AI data](/v/v4/ai/reliability/persist-ai-data/) — the semantic-cache vector store, wired from the same `@warlock.js/cache` drivers.
- [Outbound policy](/v/v4/ai/reliability/outbound-policy/) — the `OutboundPolicy` and `guardedFetch` that make `loadWeb` SSRF-safe.
