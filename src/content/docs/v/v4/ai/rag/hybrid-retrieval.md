...
`@warlock.js/ai` ships four pure functions for this — `bm25Rank`, `reciprocalRankFusion`, `hybridRank`, and `multiQuery`. They are **standalone barrel exports**, not methods on the [`ai.rag()`](/v/v4/ai/rag/run-rag/) instance (`kb.retrieve()` does dense + optional rerank only). You compose them yourself over whatever candidate set you have, so they work with any vector store. **Zero new dependencies** — all four are dependency-light and operate over the candidates you pass in.
...
- [Run RAG](/v/v4/ai/rag/run-rag/) — the `ai.rag()` pipeline; `kb.retrieve()` does dense + optional rerank, and its `ai.rag.keywordReranker` is a separate BM25-lite *reranker* (not `bm25Rank`).
- [Embed text](/v/v4/ai/rag/embed-text/) — the `ai.embedder` surface that produces the dense ranking you fuse against.
- [Define tools](/v/v4/ai/tools/define-tools/) — wrap a hybrid retriever as a tool the agent can call.
- [Persist AI data](/v/v4/ai/reliability/persist-ai-data/) — the `@warlock.js/cache` drivers that back the vector store you over-fetch from.
