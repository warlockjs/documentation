...
- `observe?: boolean | Observer` — route this run's completed report to your collectors. `true` sends it to the globally registered observers even when observe-all is off; `false` opts the run out; an `Observer` object is a run-local collector. See [the Observer seam](/v/v4/ai/observability/observe-seam/) for the value semantics.
...
All five provider adapters report this surface. See the [cost tracking recipe](/v/v4/ai/recipes/cost-tracking/) for rolling it up across a report tree.
...
| `outbound` | strict `OutboundPolicy` defaults | The [`OutboundPolicy`](/v/v4/ai/reliability/outbound-policy/) applied to a permitted remote-text fetch — https-only, host allowlist, post-DNS private-IP deny, 5 MiB cap, 10s timeout. |
...
Both denials surface as a typed `OutboundPolicyError`, never a raw throw, so you catch and report them at the boundary. See [Outbound policy](/v/v4/ai/reliability/outbound-policy/) for the shared SSRF guard.
...
A `streamingToolGuard` config recovers a tool call when the model emits the structured input as **text** in the content stream instead of as a real `tool_call`. See the [streaming tool guard recipe](/v/v4/ai/recipes/streaming-tool-guard/) for the full story.
...
Supervisor-oriented helpers — see [Run supervisor](/v/v4/ai/orchestration/run-supervisor/) for `ai.router()` (generate the routing agent) and `ai.fanOut()` (voting / self-consistency). `ai.mockRouter(decisions)` replays canned routing decisions for supervisor tests.

## When to graduate to the next rung

- **Fixed pipeline of several agents** → [Run workflow](/v/v4/ai/orchestration/run-workflow/).
- **One input routed across specialists** → [Run supervisor](/v/v4/ai/orchestration/run-supervisor/).
- **Multi-turn conversation with persistent session** → [Run orchestrator](/v/v4/ai/orchestration/run-orchestrator/) — durable session state across runs.
- **Plan generated up front, then executed** → [Planner](/v/v4/ai/architecture-concepts/planner/).

## Related

- [Define tools](/v/v4/ai/tools/define-tools/) — typed tools the agent can call.
- [Write system prompts](/v/v4/ai/prompts/write-system-prompts/) — composable prompts.
- [Handle errors](/v/v4/ai/reliability/handle-errors/) — the typed error hierarchy.
- [Streaming tool guard recipe](/v/v4/ai/recipes/streaming-tool-guard/) — recovering leaked tool calls.
