...
It is distinct from [`ai.mcp.serve`](/v/v4/ai/tools/connect-mcp/#expose-a-warlock-agent-as-an-mcp-server), which exposes a primitive as an MCP server for other MCP clients. `ai.serve` exposes it as a plain HTTP SSE endpoint for any client that speaks `text/event-stream`.
...
- [Run orchestrator](/v/v4/ai/orchestration/run-orchestrator/) — pair with `serve` for durable, multi-turn HTTP serving.
- [Connect MCP](/v/v4/ai/tools/connect-mcp/) — `ai.mcp.serve`, the MCP-server counterpart to `ai.serve`.
- [Log AI calls](/v/v4/ai/observability/log-ai-calls/) — the events `serve` streams are the same lifecycle events you can subscribe to in-process.
- [Handle errors](/v/v4/ai/reliability/handle-errors/) — how run errors surface; `serve` mirrors them onto an `error` SSE frame.
