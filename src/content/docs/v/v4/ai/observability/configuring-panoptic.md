...
There are two ways to turn Panoptic on. The [imperative `panoptic()` API](/v/v4/ai/observability/ai-panoptic/) hands you a subscriber you `attach()` / `collect()` yourself — full control, explicit wiring. This page covers the **declarative** path: set `ai.config({ panoptic: { … } })` once and let a side-effect import register the collector onto core's observe seam for you. App code never hand-wires a store, a collector, or a subscriber.
...
| `dashboard` | _unset_ | `true` uses the [dashboard defaults](/v/v4/ai/observability/local-dashboard/); an object overrides them. See [the local dashboard](/v/v4/ai/observability/local-dashboard/) for the full option set. |

### Persisting traces across a restart

By default the dashboard reads an in-memory store that's wiped on process exit. Set `cache` to a `@warlock.js/cache` `CacheDriver` and panoptic backs the dashboard with a [cache-persistent store](/v/v4/ai/observability/querying-traces/#persisting-traces-across-restarts) instead — traces are written through to the cache and **re-hydrated on the next boot**, so a restart no longer loses your trace history:
...
Declarative config does not replace `panoptic()` — it sits on top of it. Everything on the [overview page](/v/v4/ai/observability/ai-panoptic/) is intact:
...
- **The dashboard needs a queryable store.** If you set `dashboard` but pass no store among `exporters`, Panoptic creates an in-memory store and registers it for you. If you do pass one, it's reused — see [the local dashboard](/v/v4/ai/observability/local-dashboard/).
...
- [@warlock.js/ai-panoptic](/v/v4/ai/observability/ai-panoptic/) — the package overview and the imperative `panoptic()` entry point.
- [The local dashboard](/v/v4/ai/observability/local-dashboard/) — the zero-setup UI the `dashboard` option starts.
- [Querying traces](/v/v4/ai/observability/querying-traces/) — the in-memory store you register as an exporter, then `query()` / `aggregate()`.
- [What Panoptic traces](/v/v4/ai/observability/what-panoptic-traces/) — the `Trace` / `TraceSpan` shape every exporter receives.
