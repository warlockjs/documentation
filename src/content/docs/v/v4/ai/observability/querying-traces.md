...
The in-memory store is wiped on process exit. For a long-lived process — or so the [dashboard](/v/v4/ai/observability/local-dashboard/) keeps your trace history across restarts — `createCacheTraceStore(cache, options?)` persists every trace through any [`@warlock.js/cache`](/v/v4/cache/) `CacheDriver` (Redis, Postgres, file, …). It returns the **same `TraceStoreContract & ExporterContract`** as the in-memory store, plus a `ready()` hydrator — so it's a drop-in replacement, queryable identically:
...
`groupByPrompt` keys off the agent span's `agent.promptName` / `agent.promptVersion` stamps, so every run of a `name@version` prompt slices apart — the link back to the [prompt registry](/v/v4/ai/prompts/prompt-registry/).
...
If your exporter wraps a third-party SDK, lazily `await import(...)` it **inside** the `export` path (never at module top level) so it stays an optional peer dependency — copy the loader pattern from the shipped `otelExporter` / `langfuseExporter`. See [Export traces](/v/v4/ai/observability/exporter-output/) for the full exporter walkthrough.

## See also

- [What Panoptic traces](/v/v4/ai/observability/what-panoptic-traces/) — the `Trace` / `TraceSpan` shape this store retains.
- [Exporter output](/v/v4/ai/observability/exporter-output/) — sending traces to a backend instead of (or alongside) retaining them in memory.
