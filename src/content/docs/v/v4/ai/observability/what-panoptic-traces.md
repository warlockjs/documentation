...
> Capturing full conversations means raw prompts, tool payloads, and completions ride along on every span and into every exporter. Pair `fullHistory` with a `redactContent` masker in any environment where that content is sensitive, and keep the [local dashboard](/v/v4/ai/observability/local-dashboard/) loopback-only.

## Related

- [@warlock.js/ai-panoptic](/v/v4/ai/observability/ai-panoptic/) — the package overview, `panoptic()` entry point, and exporters.
- [Configuring Panoptic](/v/v4/ai/observability/configuring-panoptic/) — wire content capture declaratively via `ai.config({ panoptic })`.
- [The local dashboard](/v/v4/ai/observability/local-dashboard/) — inspect captured content in a loopback-only UI.
