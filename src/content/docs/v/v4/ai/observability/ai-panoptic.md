...
> **Prefer zero-wiring setup?** Skip the imperative `panoptic()` / `attach()` plumbing and turn observability on declaratively with [`ai.config({ panoptic })`](/v/v4/ai/observability/configuring-panoptic/) — exporters, observe-all, and an optional [zero-setup local dashboard](/v/v4/ai/observability/local-dashboard/) for eyeballing your traces in the browser. The imperative API below still powers it and remains available for fine-grained control.
...
For exactly what each exporter writes — the console tree format, the JSON-Lines record, the `gen_ai.*` attribute mapping, and the Langfuse object shapes — see [Exporter output](/v/v4/ai/observability/exporter-output/).
...
`createCacheTraceStore(cache, options?)` is the same query surface backed by any [`@warlock.js/cache`](/v/v4/cache/) `CacheDriver`, so traces **survive a process restart** — reads stay synchronous (served from an in-memory mirror), writes go through to the cache, and `ready()` re-hydrates on boot. The query / aggregate API and the persistent store are covered in [Querying traces](/v/v4/ai/observability/querying-traces/).

## Continue reading

- [Configuring Panoptic](/v/v4/ai/observability/configuring-panoptic/) — the declarative `ai.config({ panoptic })` path: `exporters`, `observeAll`, the per-flow `observe` option, and how the side-effect import registers the collector for you.
- [The local dashboard](/v/v4/ai/observability/local-dashboard/) — the zero-setup loopback UI started by `ai.config({ panoptic: { dashboard } })` (or `dashboard(store, options)`): theme, a two-pane call-tree / timeline drawer, cost heatmap, search / filter / group-by, deep-links, its options, and its read-only JSON API.
- [What Panoptic traces look like](/v/v4/ai/observability/what-panoptic-traces/) — the `Trace` / `TraceSpan` / `TraceSpanError` shape, how a `BaseReport` projects into spans, the building-block projections (`reportToTrace()`, `reportToSpan()`, `extractSpanAttributes()`), and content capture (`captureContent` / `fullHistory`).
- [Exporter output](/v/v4/ai/observability/exporter-output/) — what each of `consoleExporter`, `fileExporter`, `otelExporter`, and `langfuseExporter` emits, plus the exporter utilities (`toGenAiAttributes()`, `walkSpans()`, `totalCostUsd()`, `GEN_AI_ATTRIBUTES` / `WARLOCK_ATTRIBUTES`).
- [Querying traces](/v/v4/ai/observability/querying-traces/) — the in-memory and cache-persistent stores' `query()` / `aggregate()` API, the `TraceQuery` filter fields, the `TraceAggregate` result shape, and the exported trace-list folds (`filterTraces` / `groupBySession` / `groupByPrompt` / `rollupCost` / heatmap) the dashboard mirrors.

## Related

- [Orchestrators](/v/v4/ai/architecture-concepts/orchestrators/) — collect orchestrator turns via `collect()`.
- [Log AI calls](/v/v4/ai/observability/log-ai-calls/) — the `log`-channel side of observability.
- [Cost tracking recipe](/v/v4/ai/recipes/cost-tracking/) — reading rolled-up `usage` directly off a report.
