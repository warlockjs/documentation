...
[`@warlock.js/ai`](/v/v4/ai/). It turns OpenAI Chat Completions into
...
[`ai.image()`](/v/v4/ai/) output verb — prompt in, images out, in the
...
accounting path. See [`ai.image`](/v/v4/ai/) for the full verb surface
(options, cost-truth, `GeneratedImage`).

## Speech (text-to-speech)

`openai.speech({ name })` returns a `SpeechModelContract` for the
[`ai.speech()`](/v/v4/ai/) verb — text in, audio out.
...
[`ai.transcribe()`](/v/v4/ai/) verb — audio in, text out. Build the
...
- [`ai.image` / `ai.speech` / `ai.transcribe`](/v/v4/ai/) — the modality
  verb surfaces these factories feed.
- [Google provider](/v/v4/ai/providers/google/) — Gemini and Imagen image
  generation, and Gemini `inlineData` multimodal input.
- [API reference](/v/v4/ai/reference/api/) — every public export.
