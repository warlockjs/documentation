...
The [Orchestrators](/v/v4/ai/architecture-concepts/orchestrators/) concept page covers *why*. This page is the working surface: every config field, the per-turn result envelope, `iterate` + `resume`, drift recovery, and the production lifecycle.
...
- [Orchestrators](/v/v4/ai/architecture-concepts/orchestrators/) — the concept and mental model.
- [Run supervisor](/v/v4/ai/orchestration/run-supervisor/) — the engine the orchestrator delegates to.
- [Persist AI data](/v/v4/ai/reliability/persist-ai-data/) — checkpoint vs snapshot stores, the snapshot API.
- [Memory](/v/v4/ai/architecture-concepts/memory/) — the `memory` field.
- [Handle errors](/v/v4/ai/reliability/handle-errors/) — `OrchestratorDriftError`. A turn cancelled via its `AbortSignal` surfaces `SUPERVISOR_CANCELLED` on `result.error` (the orchestrator delegates each turn to the supervisor, so the supervisor's typed cancellation passes straight through) with `report.status: "cancelled"`.
