...
`ai.speech()` is the audio-output counterpart to [`ai.image()`](/v/v4/ai/modalities/generate-images/). Text-in / audio-out, wrapped in the **same** uniform result contract every AI verb returns — so a synthesized voicemail slots into your cost dashboards and observability traces exactly like an agent run.

This is audio **output** (TTS). For audio **input** — transcribing a WhatsApp voice note or a meeting recording — see [Transcribe audio](/v/v4/ai/modalities/transcribe-audio/).
...
- [Transcribe audio](/v/v4/ai/modalities/transcribe-audio/) — the inverse verb (`ai.transcribe`), audio → text.
- [Generate images](/v/v4/ai/modalities/generate-images/) — the sibling image-output verb (`ai.image`).
- [Run agent](/v/v4/ai/agents/run-agent/) — the chat/tool-calling verb.
