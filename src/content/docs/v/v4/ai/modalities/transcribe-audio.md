...
`ai.transcribe()` is the inverse of [`ai.speech()`](/v/v4/ai/modalities/generate-speech/). Audio-in / text-out, wrapped in the **same** uniform result contract every AI verb returns — so transcribing a support voicemail slots into your cost dashboards and observability traces exactly like an agent run.
...
This is audio **input** (STT). For audio **output** — synthesizing a voice line — see [Generate speech](/v/v4/ai/modalities/generate-speech/).
...
- [Generate speech](/v/v4/ai/modalities/generate-speech/) — the inverse verb (`ai.speech`), text → audio.
- [Generate images](/v/v4/ai/modalities/generate-images/) — the sibling image-output verb (`ai.image`).
- [Run agent](/v/v4/ai/agents/run-agent/) — hand the transcript to an agent for a reply.
