...
- [Generate images](/v/v4/ai/modalities/generate-images/) — still-image output (`ai.image`), the synchronous sibling `ai.video` mirrors.
- [Run orchestrator](/v/v4/ai/orchestration/run-orchestrator/) — the other session-shaped primitive `ai.realtime` resembles.
- [Log AI calls](/v/v4/ai/observability/log-ai-calls/) — the `observe` seam both verbs route their reports through.
- [Handle errors](/v/v4/ai/reliability/handle-errors/) — how the typed `AIError` on `result.error` and the realtime `error` event surface.
