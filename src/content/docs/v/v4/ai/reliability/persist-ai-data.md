...
The driver must support `similar()`. Without similarity → `CacheUnsupportedError`. The `defaultStore` here is the **cache driver** — distinct from `defaultSnapshotStore`. See [Attach middleware](/v/v4/ai/reliability/attach-middleware/).
...
The `CheckpointStore` is append-only (`load` / `save` / `delete` / `list?`), keyed by `(orchestrator_name, session_id, turn_index)`. See [Run orchestrator](/v/v4/ai/orchestration/run-orchestrator/) for the full session lifecycle and boot-drain pattern.

## Related

- [Run workflow](/v/v4/ai/orchestration/run-workflow/) — `snapshotStore` and `resume()`.
- [Run supervisor](/v/v4/ai/orchestration/run-supervisor/) — same on supervisor.
- [Run orchestrator](/v/v4/ai/orchestration/run-orchestrator/) — checkpoint + snapshot stores, session state.
- [Attach middleware](/v/v4/ai/reliability/attach-middleware/) — `semanticCache` middleware.
- [Handle errors](/v/v4/ai/reliability/handle-errors/) — drift errors.
