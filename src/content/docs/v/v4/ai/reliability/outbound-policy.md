...
- **Remote-text attachments** — `AgentConfig.attachmentPolicy.outbound` (or per-call `AgentExecuteOptions.attachmentPolicy`). Note remote fetch is itself default-denied — see [Attach middleware](/v/v4/ai/reliability/attach-middleware/).
- **URL skill sources** — the `policy` field on a `url` skill source. See [Runtime skills](/v/v4/ai/tools/runtime-skills/).
...
When the policy is enforced *inside* a managed surface (an agent attachment, a skill source), the `OutboundPolicyError` lands on that surface's result envelope rather than throwing past `execute()` — see [Handle errors](/v/v4/ai/reliability/handle-errors/).

## Related

- [Attach middleware](/v/v4/ai/reliability/attach-middleware/) — `AgentConfig.attachmentPolicy`, where `outbound` and `allowRemoteFetch` live.
- [Runtime skills](/v/v4/ai/tools/runtime-skills/) — URL skill sources that carry an `OutboundPolicy`.
- [Define tools](/v/v4/ai/tools/define-tools/) — tool / step code is where you most often fetch a user-supplied URL.
- [Handle errors](/v/v4/ai/reliability/handle-errors/) — how `OutboundPolicyError` surfaces on the result envelope.
- [API reference](/v/v4/ai/reference/api/) — full export list.
