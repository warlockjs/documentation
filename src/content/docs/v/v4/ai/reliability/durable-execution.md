...
- [Human in the loop](/v/v4/ai/reliability/human-in-the-loop/) — the OTHER resume: `ai.human.resume` of a gated tool call, not a crash.
- [Persist AI data](/v/v4/ai/reliability/persist-ai-data/) — the supervisor / workflow snapshot resume (the sibling `iterate`-style durability), the `ai.snapshot.{memory,pg,redis}` factories, the `SnapshotStore` contract, and the never-auto-migrated `schema()`.
- [Handle errors](/v/v4/ai/reliability/handle-errors/) — the typed `AgentDriftError` / `PlannerDriftError` / `AgentExecutionError` / `PlannerFailedError` and how `result.error` surfaces a failed run.
