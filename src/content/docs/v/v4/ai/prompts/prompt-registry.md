...
`systemPrompt().validate(options?)` is the per-builder sugar — exactly `ai.prompts.validate(this, options)`. See [Write system prompts](/v/v4/ai/prompts/write-system-prompts/#validate-this-prompt).
...
Pair `validate()` with [`agent.eval`](/v/v4/ai/testing/evaluation-and-datasets/) for a two-layer quality gate: lint the prompt text, then measure the resulting behaviour against a dataset.

## Related

- [Write system prompts](/v/v4/ai/prompts/write-system-prompts/) — the `SystemPrompt` builder, `.meta()` identity, and registry-aware `.merge()`.
- [Refine prompts](/v/v4/ai/prompts/refine-prompts/) — `systemPrompt().refined({ model, criteria, store })`, the prompt compiler; register its output as a next version to `diff` original vs refined.
- [Evaluation and datasets](/v/v4/ai/testing/evaluation-and-datasets/) — grade the *behaviour* a prompt produces, not just the text.
- [Run agent](/v/v4/ai/agents/run-agent/) — where a resolved prompt is consumed.
