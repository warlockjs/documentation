...
`.validate(options?)` is sugar over [`ai.prompts.validate(this, options)`](/v/v4/ai/prompts/prompt-registry/#validate--deterministic-check--optional-judge). It always runs the deterministic missing-placeholder check, and — when `options.judge` is a model — the Nova-safe LLM-as-judge quality pass:
...
- [Prompt registry](/v/v4/ai/prompts/prompt-registry/) — `ai.prompts`, the process-wide `name@version` store a named prompt auto-registers into, plus `validate` / `diff` / `export`.
- [Refine prompts](/v/v4/ai/prompts/refine-prompts/) — `.refined({ model, criteria, store })`, the prompt compiler: lazily rewrite this builder into a model-optimized version, pinned like a lockfile.
- [Run agent](/v/v4/ai/agents/run-agent/) — `systemPrompt` on the factory and per-call override.
- [Run workflow](/v/v4/ai/orchestration/run-workflow/) — per-step agents inherit their own system prompts.
