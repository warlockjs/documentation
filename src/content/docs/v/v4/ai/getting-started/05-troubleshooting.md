...
Same shape for every adapter — `new AnthropicSDK(...)`, `new BedrockSDK(...)`, etc. See [Pick a provider](/v/v4/ai/getting-started/03-pick-a-provider/).
...
A bare `import` is enough; any named import from the package pulls the side effect in too. See [Use AI tools](/v/v4/ai/tools/use-ai-tools/) and [Use a workspace](/v/v4/ai/tools/use-workspace/).
...
The six flags are `structuredOutput`, `vision`, `reasoning`, `promptCaching`, `audio`, `pdf` — see [Pick a provider](/v/v4/ai/getting-started/03-pick-a-provider/).
...
An actual thrown exception means something earlier broke — a malformed config, a missing required option, or a bug in your own callback — not a model failure. See [Handle errors](/v/v4/ai/reliability/handle-errors/).
...
`usage.total` (tokens) is always present; only the per-channel `usage.cost` breakdown depends on pricing. See [Cost tracking](/v/v4/ai/recipes/cost-tracking/).
...
With a token set, every request needs `Authorization: Bearer <token>` and its `Host` is checked against `allowedHosts`. See [The local dashboard](/v/v4/ai/observability/local-dashboard/).
...
See [Run orchestrator](/v/v4/ai/orchestration/run-orchestrator/) for the full recovery paths.

## Related

- [Pick a provider](/v/v4/ai/getting-started/03-pick-a-provider/) — adapter construction and capability flags.
- [Handle errors](/v/v4/ai/reliability/handle-errors/) — the `AIError` hierarchy and the `cause` chain.
- [API reference](/v/v4/ai/reference/api/) — every public export grouped by primitive.
