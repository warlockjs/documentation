...
| Export | Shape | See |
| --- | --- | --- |
| `ai.agent(config)` | `(config: AgentConfig) => AgentContract<T>` | [Run agent](/v/v4/ai/agents/run-agent/) |
| `ai.tool(config)` | `(config: ToolConfig) => ToolContract<TIn, TOut>` | [Define tools](/v/v4/ai/tools/define-tools/) |
| `ai.systemPrompt(input?)` | `() / (string) / (block[]) => SystemPrompt` | [Write system prompts](/v/v4/ai/prompts/write-system-prompts/) |
| `ai.persona(text)` | `(text: string) => PersonaContract` | [Write system prompts](/v/v4/ai/prompts/write-system-prompts/) |
| `ai.instruction(text)` | `(text: string) => InstructionContract` | [Write system prompts](/v/v4/ai/prompts/write-system-prompts/) |
| `ai.workflow(config)` | `<TIn, TOut, TState>(config) => WorkflowInstance` | [Run workflow](/v/v4/ai/orchestration/run-workflow/) |
| `ai.step(config)` | `<TIn, TState>(config) => StepDefinition` | [Run workflow](/v/v4/ai/orchestration/run-workflow/) |
| `ai.supervisor(config)` | `<TOutput>(config) => SupervisorContract` | [Run supervisor](/v/v4/ai/orchestration/run-supervisor/) |
| `ai.team(config)` | `<TOutput, TState, TMembers>(config) => SupervisorContract` (manager + members + gate sugar) | [Run supervisor](/v/v4/ai/orchestration/run-supervisor/) |
| `ai.orchestrator(config)` | `<TOutput, TState>(config) => OrchestratorContract` | [Run orchestrator](/v/v4/ai/orchestration/run-orchestrator/) |
| `ai.planner(config)` | `<TOutput>(config) => PlannerContract` | [Planner](/v/v4/ai/architecture-concepts/planner/) |
| `ai.spawnSubAgent(spec)` | `<TOutput>(spec) => Promise<AgentResult<T>>` | [Spawn sub-agent](/v/v4/ai/agents/spawn-sub-agent/) |
| `ai.image(params)` | `(params: ImageParams) => Promise<ImageResult>` — text-to-image output verb (`GeneratedImage[]`) | [OpenAI](/v/v4/ai/providers/openai/) / [Google](/v/v4/ai/providers/google/) |
| `ai.speech(params)` | `(params: SpeechParams) => Promise<SpeechResult>` — text-to-speech output verb (`GeneratedAudio`) | [OpenAI](/v/v4/ai/providers/openai/) |
| `ai.transcribe(params)` | `(params: TranscribeParams) => Promise<TranscriptionResult>` — speech-to-text input verb | [OpenAI](/v/v4/ai/providers/openai/) |
| `ai.audioFromFile(path, opts?)` | `(path, { mediaType? }) => Promise<AudioInput>` — read + package an audio file (non-AI plumbing for `ai.transcribe`) | [OpenAI](/v/v4/ai/providers/openai/) |
| `ai.audioFromBuffer(bytes, mediaType, filename?)` | `(...) => AudioInput` — package raw audio bytes (no I/O, no AI) | [OpenAI](/v/v4/ai/providers/openai/) |
| `ai.memory(config)` | `(config) => MemoryContract` | [Memory](/v/v4/ai/architecture-concepts/memory/) |
| `ai.skills(config)` | `(config: SkillsConfig) => SkillsContract` (progressive-disclosure skills library) | [Memory](/v/v4/ai/architecture-concepts/memory/) |
| `ai.rag(config)` | `(config: RagConfig) => Rag` (chunk → embed → retrieve → cite) | [Memory](/v/v4/ai/architecture-concepts/memory/) |
| `ai.rag.keywordReranker(opts?)` / `ai.rag.llmReranker(opts)` | built-in rerankers for the `reranker` slot | [Memory](/v/v4/ai/architecture-concepts/memory/) |
| `ai.router(config)` | `(config) => AgentContract` (generated routing agent) | [Run supervisor](/v/v4/ai/orchestration/run-supervisor/) |
| `ai.fanOut(unit, n, opts?)` | spread one unit into N keyed intents | [Run supervisor](/v/v4/ai/orchestration/run-supervisor/) |
| `ai.batch(exec, items, opts?)` | run an executable over a dataset, bounded concurrency | [Run agent](/v/v4/ai/agents/run-agent/) |
| `ai.fallbackModel(models, opts?)` | ordered model list with failover | [Run agent](/v/v4/ai/agents/run-agent/) |
| `ai.prompts` | `PromptsManagerContract` — the process-wide `name@version` prompt registry (resolve / merge / validate / diff / export by name) | [Prompt registry](/v/v4/ai/prompts/prompt-registry/) |
| `ai.prompt(options?)` | thin facade over `ai.prompts`; the options form returns an isolated legacy `PromptRegistryContract` | [Prompt registry](/v/v4/ai/prompts/prompt-registry/) |
| `ai.dataset(options)` | `<TOutput>(options: DatasetOptions) => DatasetContract` (filterable / shardable eval cases) | [Run agent](/v/v4/ai/agents/run-agent/) |
| `ai.vcr(model, options)` | `(model: ModelContract, options: VcrOptions) => VcrModel` (record / replay decorator) | [Run agent](/v/v4/ai/agents/run-agent/) |
| `ai.eval.{exact,contains,predicate,judge}` | built-in `agent.eval()` scorers | [Run agent](/v/v4/ai/agents/run-agent/) |
| `ai.eval.{toJUnit,toJSON,fromJSON}` | CI reporters / round-trip serialization over a finished `EvalReport` | [Run agent](/v/v4/ai/agents/run-agent/) |
| `ai.mockRouter(decisions, opts?)` | canned routing decisions for tests | [Run supervisor](/v/v4/ai/orchestration/run-supervisor/) |
| `ai.config(partial)` | `(partial: Partial<AIConfig>) => AIConfig` | [Persist AI data](/v/v4/ai/reliability/persist-ai-data/) |
| `ai.checkpoint.{memory,pg,redis}()` | orchestrator session checkpoint stores | [Run orchestrator](/v/v4/ai/orchestration/run-orchestrator/) |
| `ai.snapshot.{memory,pg,redis}()` | workflow / supervisor / orchestrator snapshot stores | [Persist AI data](/v/v4/ai/reliability/persist-ai-data/) |
| `ai.systemPrompt.fromFile(path)` | seed a system prompt from a file | [Run agent](/v/v4/ai/agents/run-agent/) |
| `ai.human.approval(opts)` | `(opts: HumanApprovalOptions) => AgentMiddleware` — the `tool.before` approval gate | [Attach middleware](/v/v4/ai/reliability/attach-middleware/) |
| `ai.human.resume(id, decision, opts)` | `<TOutput>(...) => Promise<ResumeResult>` — out-of-process durable resume | [Attach middleware](/v/v4/ai/reliability/attach-middleware/) |
| `ai.human.interrupt.{memory,pg,redis}()` | durable `InterruptStore` factories | [Attach middleware](/v/v4/ai/reliability/attach-middleware/) |
| `ai.guardrail(opts)` | `(opts: GuardOptions) => AgentMiddleware` — content-intelligence guardrail suite | [Attach middleware](/v/v4/ai/reliability/attach-middleware/) |
| `ai.guardrail.{pii,topic,injection,moderation}(opts?)` | built-in detector factories for the `input` / `output` / `tool` arrays | [Attach middleware](/v/v4/ai/reliability/attach-middleware/) |
| `ai.middleware.budget(opts)` | budget cap middleware | [Attach middleware](/v/v4/ai/reliability/attach-middleware/) |
| `ai.middleware.guardrail(opts)` | pre/post check middleware (the legacy guardrail seed) | [Attach middleware](/v/v4/ai/reliability/attach-middleware/) |
| `ai.middleware.semanticCache(opts)` | two-tier cache middleware | [Attach middleware](/v/v4/ai/reliability/attach-middleware/) |
| `ai.middleware.compose(...sources)` | flatten middleware sources | [Attach middleware](/v/v4/ai/reliability/attach-middleware/) |
| `ai.middleware.forTool(name, mw)` | scope tool hooks to a tool name | [Attach middleware](/v/v4/ai/reliability/attach-middleware/) |
...
| `WorkflowSnapshot` | Persisted shape — see [Persist AI data](/v/v4/ai/reliability/persist-ai-data/). |
...
| `bm25Rank(query, docs)` | BM25 lexical ranking over a candidate set (`k1 = 1.5`, `b = 0.75`; zero-score docs dropped). Pure; no global index. See [Hybrid retrieval](/v/v4/ai/rag/hybrid-retrieval/). |
...
`ai.streamObject` streams token deltas, progressively-parsed partial snapshots, and a final strictly-validated object. See [Stream structured output](/v/v4/ai/agents/stream-structured-output/).
...
The output/input-modality track — image, speech, and transcription verbs that share the uniform never-throws `{ data, error, usage, report }` envelope with cost-truth and `observe` routing. Models come from an adapter's `image()` / `speech()` / `transcribe()` factory. See [OpenAI](/v/v4/ai/providers/openai/) and [Google](/v/v4/ai/providers/google/).
...
`ai.prompts` is the process-wide registry of named, versioned `systemPrompt(...)` builders keyed by `name@version`. A named prompt auto-registers; resolve / merge / validate / diff by name. `prompts(options?)` builds an isolated registry. See [Prompt registry](/v/v4/ai/prompts/prompt-registry/).
...
Turn any streaming primitive (agent / supervisor / orchestrator) into a `node:http` handler that streams its run to the client as Server-Sent Events. See [Serve over SSE](/v/v4/ai/agents/serve-over-sse/).
...
All extend `AIError`. Stable `code` strings listed in [Handle errors](/v/v4/ai/reliability/handle-errors/).
...
A shared trust-boundary foundation used by every server-side outbound request (attachment fetches, URL skill sources, RAG loaders) plus secret-scrubbing for logs / errors. See [Outbound policy](/v/v4/ai/reliability/outbound-policy/) and [Redact secrets](/v/v4/ai/reliability/redact-secrets/).
...
- [Pick a provider](/v/v4/ai/getting-started/03-pick-a-provider/) — adapter contract.
- [The basics](/v/v4/ai/agents/run-agent/) — must-know surface in narrative form.
- [Handle errors](/v/v4/ai/reliability/handle-errors/) — full error semantics.
