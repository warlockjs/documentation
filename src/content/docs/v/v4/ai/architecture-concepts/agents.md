...
This page is the mental model. For the API surface see [Run agent](/v/v4/ai/agents/run-agent/).
...
2. Every tool call this trip is `mode: "silent"` — see [Define tools](/v/v4/ai/tools/define-tools/).
...
There's a side-channel — `ctx.artifacts` — for system-only data the model should never see (renderable blocks, citations, telemetry). See [Define tools](/v/v4/ai/tools/define-tools/).
...
If you want a long-lived conversation that "remembers" across runs, the [`ai.orchestrator()`](/v/v4/ai/architecture-concepts/orchestrators/) primitive owns this for you — durable session state, history windowing, and optional [memory](/v/v4/ai/architecture-concepts/memory/). You still hold the raw transcript and replay it via `history`; the orchestrator persists the *session state* around it.

## Related

- [Run agent](/v/v4/ai/agents/run-agent/) — the API surface in depth.
- [Define tools](/v/v4/ai/tools/define-tools/) — tool authoring.
- [Write system prompts](/v/v4/ai/prompts/write-system-prompts/) — composable prompts.
- [Workflows](/v/v4/ai/architecture-concepts/workflows/) — when one agent isn't enough.
