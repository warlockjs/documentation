...
This page is the mental model. For the API surface see [Run workflow](/v/v4/ai/orchestration/run-workflow/).
...
Reach for a supervisor when the *shape* changes per call. Reach for an [orchestrator](/v/v4/ai/architecture-concepts/orchestrators/) when the *session* matters across runs.
...
Drift is the framework refusing to silently corrupt your data. You then choose: discard the snapshot, force-resume (escape hatch for trivial edits), or migrate manually. See [Persist AI data](/v/v4/ai/reliability/persist-ai-data/).
...
- **Unknown shape at author time** — reach for [`ai.planner()`](/v/v4/ai/architecture-concepts/planner/) (LLM generates the plan over your capabilities), or model it as a supervisor where the router decides.
- **Quality loop until goal met** — use `ai.supervisor()` with `evaluate`.
- **Multi-turn conversation with persistent session** — [`ai.orchestrator()`](/v/v4/ai/architecture-concepts/orchestrators/).
- **Iterate a runtime list of items** — wrap a workflow with `ai.batch()` utility.

## Related

- [Run workflow](/v/v4/ai/orchestration/run-workflow/) — the API surface.
- [Supervisors](/v/v4/ai/architecture-concepts/supervisors/) — the next rung up.
- [Orchestrators](/v/v4/ai/architecture-concepts/orchestrators/) — stateful sessions across runs.
- [Planner](/v/v4/ai/architecture-concepts/planner/) — generate-then-execute over capabilities.
- [Persist AI data](/v/v4/ai/reliability/persist-ai-data/) — snapshot resume + drift handling.
