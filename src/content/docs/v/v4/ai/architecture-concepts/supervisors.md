...
This page is the mental model. For the API surface see [Run supervisor](/v/v4/ai/orchestration/run-supervisor/).
...
- **Long-running conversation** — [`ai.orchestrator()`](/v/v4/ai/architecture-concepts/orchestrators/) wraps a supervisor with durable session state across runs.
- **Plan generated up front** — [`ai.planner()`](/v/v4/ai/architecture-concepts/planner/) asks an LLM to plan over your capabilities, then runs the plan.

## Related

- [Run supervisor](/v/v4/ai/orchestration/run-supervisor/) — the API surface.
- [Orchestrators](/v/v4/ai/architecture-concepts/orchestrators/) — the stateful primitive a supervisor delegates into.
- [Planner](/v/v4/ai/architecture-concepts/planner/) — generate-then-execute over capabilities.
- [Define tools](/v/v4/ai/tools/define-tools/) — `ctx.artifacts` side-channel.
- [Persist AI data](/v/v4/ai/reliability/persist-ai-data/) — snapshot resume.
