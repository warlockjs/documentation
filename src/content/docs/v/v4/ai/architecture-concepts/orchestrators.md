...
`force: true` bypasses the check for edits you know are safe. This mirrors the supervisor / workflow drift model — see [Run orchestrator](/v/v4/ai/orchestration/run-orchestrator/) for the recovery paths.
...
An orchestrator can recall and accumulate **memory** across turns — see [Memory](/v/v4/ai/architecture-concepts/memory/). Wire a `memory` store and the orchestrator recalls relevant memories before routing (injecting them into `ctx.context[injectKey]`) and remembers the settled turn outcome afterward:
...
- [Run orchestrator](/v/v4/ai/orchestration/run-orchestrator/) — the full config surface, resume, drift recovery, production boot-drain.
- [Supervisors](/v/v4/ai/architecture-concepts/supervisors/) — the engine the orchestrator delegates to.
- [Memory](/v/v4/ai/architecture-concepts/memory/) — working + semantic recall across turns.
- [Persist AI data](/v/v4/ai/reliability/persist-ai-data/) — checkpoint vs snapshot stores.
