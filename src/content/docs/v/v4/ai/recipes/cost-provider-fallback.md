...
- [Pick a provider](/v/v4/ai/getting-started/03-pick-a-provider/) — one SDK per provider, capabilities, and `ModelPricing`.
- [Handle errors](/v/v4/ai/reliability/handle-errors/) — the `AIErrorCode` union and which codes are transient.
- [Cost per tenant](/v/v4/ai/recipes/cost-aggregate-per-tenant/) — aggregating the usage a failed-over run still reports.
