...
- [Pick a provider](/v/v4/ai/getting-started/03-pick-a-provider/) — `ModelPricing`, the token channels, and capability flags.
- [Cost tracking](/v/v4/ai/recipes/cost-tracking/) — the per-session dashboard view of the same data.
- [Run agent](/v/v4/ai/agents/run-agent/) — the `usage` / `report` envelope `execute()` returns.
