...
- [Autonomous task runner](/v/v4/ai/recipes/planner-autonomous-task-runner/) — the planner basics this recipe builds on.
- [Cost tracking](/v/v4/ai/recipes/cost-tracking/) — configuring the pricing that `budget.maxCostUSD` reads.
- [Tool-calling agent](/v/v4/ai/recipes/tool-calling-agent/) — the tools (like `web_search`) you hand a spawned sub-agent.
