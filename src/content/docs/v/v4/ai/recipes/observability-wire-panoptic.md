...
`@warlock.js/ai-panoptic`'s only hard peer is `@warlock.js/ai` — the in-memory store and the console exporter pull in no extra SDK. (OpenTelemetry and Langfuse are optional peers; see the [export recipe](/v/v4/ai/recipes/observability-export-otel-langfuse/).)
...
The orchestrator is the one exception: its events carry no result, so you feed its report in directly. See the [cost-dashboard recipe](/v/v4/ai/recipes/observability-trace-cost-dashboard/) for that path via `observe.collect(result.report)`.
...
- [Observability — export to OTel & Langfuse](/v/v4/ai/recipes/observability-export-otel-langfuse/) — ship the same traces to an external backend.
- [Observability — trace cost dashboard](/v/v4/ai/recipes/observability-trace-cost-dashboard/) — per-run cost and latency from the collector.
- [Cost tracking](/v/v4/ai/recipes/cost-tracking/) — the `usage.cost` per-channel breakdown Panoptic rolls up.
