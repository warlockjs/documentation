...
- **Add a retention sink alongside the wire exporters.** Register an in-memory store (or a custom `TraceStoreContract`) in the same `exporters` array to keep runs queryable locally even while they ship out — see [wire Panoptic](/v/v4/ai/recipes/observability-wire-panoptic/).

## Related

- [Observability — wire Panoptic](/v/v4/ai/recipes/observability-wire-panoptic/) — attach the subscriber and retain traces in the in-memory store.
...
- [Cost tracking](/v/v4/ai/recipes/cost-tracking/) — the `usage.cost` breakdown the exporters map to `warlock.cost.usd` / Langfuse usage.
