...
- [Architecture — Agents](/v/v4/ai/architecture-concepts/agents/) — the trip loop, tools, and the `AgentResult` envelope every rung shares.
- [Architecture — Workflows](/v/v4/ai/architecture-concepts/workflows/) — steps, state, retries, and resume-from-snapshot.
- [Architecture — Supervisors](/v/v4/ai/architecture-concepts/supervisors/) — `route` vs `router`, intent dispatch, and the `evaluate` verdict.
- [Architecture — Orchestrators](/v/v4/ai/architecture-concepts/orchestrators/) — sessions, checkpoints, compaction, and the per-turn lifecycle.
- [Architecture — Planner](/v/v4/ai/architecture-concepts/planner/) — plan generation over capabilities and step-by-step execution.
- [Run a team](/v/v4/ai/orchestration/run-team/) — a manager, role-named members, and the built-in quality / verify gate.
- [Run RAG](/v/v4/ai/rag/run-rag/) — chunk, embed, store, retrieve, rerank, and cite over your own corpus.
- [Recipe — Basic agent](/v/v4/ai/recipes/basic-agent/) — the rung-1 starting point.
- [Recipe — Content pipeline workflow](/v/v4/ai/recipes/workflow-content-pipeline/) — a fixed sequence, end to end.
- [Recipe — Support triage supervisor](/v/v4/ai/recipes/supervisor-support-triage/) — dynamic dispatch with a router and `evaluate` loop.
- [Recipe — Stateful support bot](/v/v4/ai/recipes/orchestrator-stateful-support-bot/) — a multi-turn session with persisted state.
- [Recipe — Autonomous task runner](/v/v4/ai/recipes/planner-autonomous-task-runner/) — a planner that generates and runs its own plan.
