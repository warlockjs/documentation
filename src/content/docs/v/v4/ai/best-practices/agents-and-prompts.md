...
- [Architecture — Agents](/v/v4/ai/architecture-concepts/agents/) — the trip loop, the report tree, and how output parsing and repair fit in.
...
- [Recipe — Extract structured data with self-repair](/v/v4/ai/recipes/agent-extract-structured-data/) — `output` + `repair` parsing a messy resume into a typed record.
- [Recipe — Extract fields from a document image](/v/v4/ai/recipes/agent-vision-document-extraction/) — attachments and the vision capability gate, end to end.
- [Recipe — A multi-tool research agent](/v/v4/ai/recipes/agent-multi-tool-research-agent/) — several tools in one `tools` array and the bounded trip loop.
- [Recipe — A basic agent](/v/v4/ai/recipes/basic-agent/) — the smallest agent + system prompt, if you're starting from zero.
