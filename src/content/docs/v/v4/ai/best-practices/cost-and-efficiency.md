...
Reserve the router loop for messages that *genuinely* need several specialists in sequence (billing **and** tech **and** a resolver). See the [classifier fast-path recipe](/v/v4/ai/recipes/supervisor-classifier-fast-path/) for the cheap path and the [support-triage recipe](/v/v4/ai/recipes/supervisor-support-triage/) for the full loop — including the cost ladder that tells you which rung your problem is actually on.
...
> Keep the semantic cache **outermost** in the middleware chain so a hit short-circuits before any inner middleware runs, and so a guardrail rejection in `trip.after` keeps a bad response out of the store. `0.95` is a solid starting threshold for question-answering; move it only with hit-quality data, never to chase a hit-rate number. See the [RAG with cache similarity recipe](/v/v4/ai/recipes/rag-with-cache-similarity/) for the full one-driver setup.
...
> Bounded concurrency is the point: it keeps you under provider rate limits while still running far faster than a serial loop. Tune it to your account's limits, not to "as high as possible." See the [batch classify recipe](/v/v4/ai/recipes/dx-batch-classify-dataset/).
...
- [Recipe — Classifier fast-path triage](/v/v4/ai/recipes/supervisor-classifier-fast-path/) — the cheapest triage: one classify, one specialist.
- [Recipe — Support triage supervisor](/v/v4/ai/recipes/supervisor-support-triage/) — the full router loop and the triage cost ladder.
- [Recipe — Budget caps and SLO contracts](/v/v4/ai/recipes/cost-budgets-and-slo/) — hard caps, the SLO contract, and the soft-fallback signal.
- [Recipe — Stateful support bot](/v/v4/ai/recipes/orchestrator-stateful-support-bot/) — multi-turn sessions with `historyWindow` and compaction.
- [Recipe — RAG with cache similarity](/v/v4/ai/recipes/rag-with-cache-similarity/) — one cache driver for retrieval, snapshots, and response caching.
- [Recipe — Track cost](/v/v4/ai/recipes/cost-tracking/) — reading the `Usage.cost` breakdown off a result.
- [Architecture — Middleware](/v/v4/ai/architecture-concepts/middleware/) — the onion model, install order, and the budget/cache built-ins.
- [Architecture — Orchestrators](/v/v4/ai/architecture-concepts/orchestrators/) — the session lifecycle that `historyWindow` and `summarize` bound.
