...
> Panoptic's rollups follow the same honesty rule: `aggregate().cost` stays `undefined` until at least one priced run lands, and an unpriced child can never erase the cost of priced siblings — the merge uses the framework's own cost-rollup logic. So one unpriced model in the mix won't zero out your dashboard; it just won't contribute. See the [cost tracking recipe](/v/v4/ai/recipes/cost-tracking/) for the `pricing` shape and resolution order.
...
> The root span's `usage` is the whole-run rollup; each child's `usage` is its own subtree's rollup. That is what makes a span tree a flame graph for cost: the expensive node is the one whose own rollup dominates its parent's. For a cost-and-latency dashboard built entirely on `query` / `aggregate` / `walkSpans`, see the [trace cost dashboard recipe](/v/v4/ai/recipes/observability-trace-cost-dashboard/).
...
- [Recipe — Wire Panoptic onto a run](/v/v4/ai/recipes/observability-wire-panoptic/) — attach the subscriber and retain traces in the in-memory store.
- [Recipe — Export to OTel & Langfuse](/v/v4/ai/recipes/observability-export-otel-langfuse/) — ship the same traces to an external backend with optional-peer install notes.
- [Recipe — Trace cost & latency dashboard](/v/v4/ai/recipes/observability-trace-cost-dashboard/) — per-run cost, p95 latency, and failure cost from the collector.
- [Recipe — Aggregate cost per tenant](/v/v4/ai/recipes/cost-aggregate-per-tenant/) — read `Usage.cost` / `cachedTokens` / `reasoningTokens` into a tenant ledger.
- [Recipe — Cost tracking](/v/v4/ai/recipes/cost-tracking/) — the `ModelPricing` registry and the `Usage.cost` per-channel breakdown.
- [Best Practices — Cost and efficiency](/v/v4/ai/best-practices/cost-and-efficiency/) — the levers (tiering, caching, budgets) that lower the spend you're now measuring.
- [Architecture — Middleware](/v/v4/ai/architecture-concepts/middleware/) — the pipeline `panoptic().middleware()` plugs into as an alternative feed path.
