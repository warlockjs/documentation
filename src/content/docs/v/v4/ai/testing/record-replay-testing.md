...
It intercepts only `complete()` / `stream()` — the single seam every agent trip funnels through — and delegates `name`, `provider`, `capabilities`, and `pricing` to the inner model untouched, so cost accounting is unchanged. Because it depends only on `ModelContract`, it works with any adapter and composes *below* [`ai.fallbackModel`](/v/v4/ai/best-practices/resilience/).
...
- Use **`ai.vcr`** when you want a test to exercise the **real model's actual output** (real tool-call shapes, real structured output) but run it deterministically and offline thereafter — integration-flavoured tests, and the live half of an [eval suite](/v/v4/ai/testing/evaluation-and-datasets/) you want repeatable.
- Use `MockSDK` / `ai.mockRouter` / `mockAgent` (see [Testing and evals](/v/v4/ai/best-practices/testing-and-evals/)) for pure **wiring** tests — routing, step order, convergence — where you never want a real call at all and can assert against fabricated responses.
...
- [Evaluation and datasets](/v/v4/ai/testing/evaluation-and-datasets/) — make the live eval suite repeatable by replaying its model calls.
- [Testing and evals](/v/v4/ai/best-practices/testing-and-evals/) — the wiring-vs-quality split and the mocking primitives VCR complements.
- [Resilience and error handling](/v/v4/ai/best-practices/resilience/) — `ai.fallbackModel`, which VCR composes below.
