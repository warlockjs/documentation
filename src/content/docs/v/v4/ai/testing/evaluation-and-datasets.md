...
Gate the live quality suite behind `describe.runIf(hasKeys)` so the fast, offline wiring tests never need API keys — see [Testing and evals](/v/v4/ai/best-practices/testing-and-evals/) for the wiring-vs-quality split.

## Related

- [Testing and evals](/v/v4/ai/best-practices/testing-and-evals/) — the philosophy: split deterministic wiring tests from live quality evals; seed cases from production failures.
- [Record / replay testing](/v/v4/ai/testing/record-replay-testing/) — make the live eval suite deterministic and free by replaying recorded model calls.
- [Prompt registry](/v/v4/ai/prompts/prompt-registry/) — `validate()` lints prompt *text*; `agent.eval` measures the *behaviour* it produces.
